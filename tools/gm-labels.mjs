...
for (const g of ['arum','alocasia','amorphophallus']) {
  ...
  console.log(MODE, g.padEnd(15), await p.evaluate(() => {
    ...
    const labs = [...svg.querySelectorAll('.apgm-maplabel')].map(t=>t.textContent);
    const leads = svg.querySelectorAll('.apgm-maplead').length;
    return 'labels=[' + labs.join(',') + '] leads=' + leads;
  }));
  const svgH = await p.$('.apgm svg');
  await svgH.scrollIntoViewIfNeeded(); await p.waitForTimeout(400);
  await svgH.screenshot({ path: SP + 'lab-' + MODE + '-' + g + '.png', animations:'disabled' });
  await p.close();
}
await b.close();
