...
let sJs = 0, sHd = 0;
await ctx.route("**/footer.js*", (r) => { sJs++; r.fulfill({ status: 200, contentType: "application/javascript", body: js }); });
await ctx.route("**/shapes-hd.json*", (r) => { sHd++; r.fulfill({ status: 200, contentType: "application/json", body: hd }); });
...
ok(sJs > 0, `local bundle served (${sJs})`);
ok(sHd > 0, `local shapes-hd served (${sHd}) — the HD feed IS exercised here`);
