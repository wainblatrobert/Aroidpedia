#!/usr/bin/env python3
"""Validate climate.json 1.10.0 against the 1.9.0 baseline.
...
old = json.load(open("climate-1.9.0-baseline.json", encoding="utf-8"))
...
stripped = copy.deepcopy(np_)
kop_missing = []
for k, e in stripped.items():
    for v in VARS:
        if v in e:
            if "koppen" not in e[v]:
                kop_missing.append((k, v))
            e[v].pop("koppen", None)
moved = [k for k in op if stripped.get(k) != op[k]]
print("2. minus `koppen`, all 919 deep-equal to 1.9.0:",
...
for k in ["Brazil North", "West Himalaya", "Türkiye", "Mexico Southwest",
          "Borneo", "Thailand"]:
    e = np_.get(k)
    if not e:
        continue
