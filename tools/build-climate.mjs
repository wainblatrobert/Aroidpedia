...
(async () => {
  ...
  if (onlyMap.length || onlyClim.length) {
    ...
    console.error('  places.json, so a divergence means shapes.json is STALE — rerun');
    console.error('  build-shapes first. Nothing was written.');
    process.exit(1);
  }
  ...
  for (const t of ['Borneo', 'Ecuador', 'Japan', 'China South-Central', 'Barbados', 'Himalaya']) {
    const e = out[t];
    if (!e) continue;
    const v = e.ff || e.all;
    ...
    console.log(`  ${t.padEnd(20)} n=${String(e.n).padStart(5)} ff=${(e.ffShare * 100).toFixed(0).padStart(3)}% warm=${(e.warmShare * 100).toFixed(0).padStart(3)}%  ` +
      ...
      `zones ${v.zones.map(z => z[0] + ' ' + Math.round(z[1] * 100) + '%').join(', ')}  [${e.method}]`);
  }
})();
