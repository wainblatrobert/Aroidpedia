/* AROIDPEDIA footer bundle — built 2026-08-18 from FILE v104 by build-footer-bundle v1.
...
(function(){
  ...
  window.__apFooterBundle = "v104";
  if (window.console && console.info) console.info("[footer bundle] FILE v104 (hosted footer.js)");
...
(function(){
  ...
  function upgradeShapes(data, facts, parsed){
    ...
    var S = (parsed && parsed.sections) || {};
    var distText = S.distribution
      ? squash(S.distribution.nodes.map(function(n){ return n.textContent; }).join(" "))
      : "";
    if (!distText) return Promise.resolve(data);
    var hay = " " + distText.toUpperCase() + " ";
    var known = data.shapes || {}, dots = data.dots || {};
    var need = facts.tags.some(function(t){
      t = String(t || "").replace(/\s*\(?\?\)?\s*$/, "");
      if (!t || known[t] || dots[t]) return false;
      return hay.indexOf(t.toUpperCase()) >= 0;
    });
    if (!need) return Promise.resolve(data);
  ...
  function boot(){
    ...
    mounts.forEach(function(mount){
      ...
      mount.setAttribute("data-apsc-version", "card-v86-file-v104");
      if (window.console && console.info){
