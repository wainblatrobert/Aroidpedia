/* AROIDPEDIA footer bundle — built 2026-08-21 from FILE v146 by build-footer-bundle v1.
...
(function(){
  ...
  window.__apFooterBundle = "v146";
  if (window.console && console.info) console.info("[footer bundle] FILE v146 (hosted footer.js)");
...
(function(){
  ...
  var CFG = {
    ...
    CLAB_LEAD_PX : 6,      /* gap between the map edge and a margin label */
  ...
  function buildMap(data, hits, doubtful, wash, subparent){
    ...
    order.forEach(function(name){
      ...
      if (onSet[name]){ var _bb = bboxOfPath(d); if (_bb) PATHBB[name] = _bb; }
    ...
    (function(){
      ...
      Object.keys(onSet).forEach(function(name){
        ...
        var b = by[c];
        if (!b) by[c] = bb.slice();
        else { if(bb[0]<b[0])b[0]=bb[0]; if(bb[1]<b[1])b[1]=bb[1];
               if(bb[2]>b[2])b[2]=bb[2]; if(bb[3]>b[3])b[3]=bb[3]; }
      });
      var names = Object.keys(by);
      if (names.length < 2 || names.length > CFG.CLAB_MAX) return;
      /* widest first: a long name is the hardest to place, and placing
         it while the map is still empty gives it the best chance */
      names.sort(function(a,b){ return b.length - a.length; });
      names.forEach(function(c){
        var g = document.createElementNS(NS,"g");
        g.setAttribute("class","apsc-clab");
        g.setAttribute("data-bb", by[c].join(" "));
        var ln = document.createElementNS(NS,"line");
    ...
    function sizeDots(){
      ...
      /* -- COUNTRY LABELS, same pixel discipline as the marker -------
         Two placements, and the choice is MEASURED, never guessed:
         a country wide enough for "Mali" is not wide enough for
         "Burkina Faso".

         INSIDE, when the name genuinely fits within the lit area.
         Otherwise a MARGIN CALLOUT: the label goes to the left or right
         edge of the map with a leader back to the country. The first
         cut set outside labels just beyond each country's own bbox,
         which works when countries are far apart and fails badly when
         they are not - across West Africa "IVORY COAST" landed on top
         of Ghana, labelling the wrong shape. Pushing them to the margin
         keeps every name off the map body and makes the leader do the
         work of pointing, which is what a leader is for. */
      var cfs = px(CFG.CLAB_FS_PX), cgap = px(CFG.CLAB_LEAD_PX);
      var midX = vb[0] + W / 2, vbTop = vb[1], vbBot = vb[1] + vb[3];
      var rows = [];
      [].slice.call(svg.querySelectorAll("g.apsc-clab")).forEach(function(g){
        ...
        if (!tw) tw = String(tx.textContent||"").length * cfs * 0.68;
        rows.push({ g: g, tx: tx, ln: g.querySelector("line"), bb: bb, tw: tw,
                    cx: (bb[0]+bb[2])/2, cy: (bb[1]+bb[3])/2,
                    area: (bb[2]-bb[0]) * (bb[3]-bb[1]) });
      });

      var pad = cfs * 0.5, sideRows = { L: [], R: [] }, outs = [];
      rows.forEach(function(r){
        var fits = (r.tw + pad*2) <= (r.bb[2]-r.bb[0]) && (cfs*1.5) <= (r.bb[3]-r.bb[1]);
        if (fits){
          r.g.setAttribute("class", "apsc-clab apsc-clab--in");
          r.tx.setAttribute("x", r.cx.toFixed(4));
          r.tx.setAttribute("y", (r.cy + cfs*0.35).toFixed(4));
          r.tx.setAttribute("text-anchor", "middle");
        } else {
          r.g.setAttribute("class", "apsc-clab");
          outs.push(r);
        }
      });

      /* v124: ASSIGN SIDES GEOGRAPHICALLY, NOT BY THE MAP'S MIDLINE.
         Splitting on the viewBox centre piled seven names into one
         column when the lit countries happened to sit east of it, and
         a central country ("Ivory Coast") got a leader crossing the
         whole map. Sorting the countries by longitude and giving the
         western half to the left column and the eastern half to the
         right balances the columns AND makes the layout read as
         geography: the left margin lists the western countries north
         to south, the right margin the eastern ones. */
      outs.sort(function(a,b){ return a.cx - b.cx; });
      var half = Math.ceil(outs.length / 2);
      outs.forEach(function(r, i){ sideRows[i < half ? "L" : "R"].push(r); });

      /* stack each margin column in latitude order and force a minimum
         gap, so names never overlap and never cross their own leaders */
      ["L","R"].forEach(function(side){
        var list = sideRows[side];
        if (!list.length) return;
        /* WARNING: THE COLUMN CAN RUN OUT OF MAP. On scroll the panel
           collapses and the svg goes from ~350px to 170px, but the type
           stays the same size on purpose - so a column that fitted at
           rest can no longer fit, and the old code just let the names
           overlap. Work out how many rows the height allows and DROP
           the rest, smallest lit area first: a reader loses the label
           for the country contributing least, instead of losing two
           names to a collision. */
        var step = cfs * 1.7;
        var roomFor = Math.max(1, Math.floor(((vbBot - cfs) - (vbTop + cfs)) / step) + 1);
        if (list.length > roomFor){
          list.sort(function(a,b){ return b.area - a.area; });
          list.slice(roomFor).forEach(function(r){ r.g.style.display = "none"; });
          list = list.slice(0, roomFor);
        }
        list.sort(function(a,b){ return a.cy - b.cy; });
        var y = null;
        list.forEach(function(r){
          y = (y === null) ? r.cy : Math.max(r.cy, y + step);
          r.y = y;
        });
        /* if the column overruns the bottom, slide the whole run up */
        var over = list[list.length-1].y - (vbBot - cfs);
        if (over > 0) list.forEach(function(r){ r.y -= over; });
        var under = (vbTop + cfs) - list[0].y;
        if (under > 0) list.forEach(function(r){ r.y += under; });
        list.forEach(function(r){
          var x = (side === "L") ? (vb[0] + cgap) : (vb[0] + W - cgap);
          r.tx.setAttribute("x", x.toFixed(4));
          r.tx.setAttribute("y", r.y.toFixed(4));
          r.tx.setAttribute("text-anchor", side === "L" ? "start" : "end");
          if (r.ln){
            /* leader runs from the end of the text to the near edge of
               the country, so it reads as pointing rather than crossing */
            var lx = (side === "L") ? (x + r.tw + pad) : (x - r.tw - pad);
            /* aim at the CENTRE of the lit area, not the near edge. An
               edge is shared: Ivory Coast's right edge IS Ghana's left
               edge, so edge-terminated leaders for neighbours end a few
               pixels apart and label each other's country. */
            var tgt = r.cx;
            r.ln.setAttribute("x1", lx.toFixed(4));
            r.ln.setAttribute("y1", (r.y - cfs*0.3).toFixed(4));
            r.ln.setAttribute("x2", tgt.toFixed(4));
            r.ln.setAttribute("y2", r.cy.toFixed(4));
          }
        });
      });

      /* -- FINAL GUARD: MEASURE WHAT WAS ACTUALLY DRAWN --------------
         Everything above reasons per column, and that is blind to the
         one collision that matters most: the LEFT column running into
         the RIGHT one. When the panel collapses, the type stays the
         same size in PIXELS, which means it grows in USER units as the
         map shrinks - so "BURKINA FASO" eventually spans more than
         half the map and meets a name coming the other way. No amount
         of per-column arithmetic sees that.

         So stop predicting and measure. Walk the drawn labels largest
         lit area first, keep each one that clears everything kept so
         far, and hide the rest. A missing name is a small loss; two
         names on top of each other is an unreadable map.

         Pre-mount there is no layout and every rect is zero, so this
         does nothing on the first pass - the ResizeObserver run right
         after mount is the one that counts. */
      var kept = [];
      rows.slice().sort(function(a,b){ return b.area - a.area; }).forEach(function(r){
        if (r.g.style.display === "none") return;
        var box = r.tx.getBoundingClientRect();
        if (!box.width && !box.height) return;
        var clash = false;
        for (var i = 0; i < kept.length; i++){
          var q = kept[i];
          if (box.left < q.right && q.left < box.right &&
              box.top < q.bottom && q.top < box.bottom){ clash = true; break; }
        }
        if (clash) r.g.style.display = "none";
        else kept.push(box);
      });
    }
  ...
  function boot(){
    ...
    mounts.forEach(function(mount){
      ...
      mount.setAttribute("data-apsc-version", "card-v124-file-v146");
      if (window.console && console.info){
