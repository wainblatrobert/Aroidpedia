/* AROIDPEDIA footer bundle — built 2026-08-21 from FILE v147 by build-footer-bundle v1.
...
(function(){
  ...
  window.__apFooterBundle = "v147";
  if (window.console && console.info) console.info("[footer bundle] FILE v147 (hosted footer.js)");
...
(function(){
  ...
  function buildMap(data, hits, doubtful, wash, subparent){
    ...
    function sizeDots(){
      ...
      /* -- COUNTRY LABELS: point-feature label placement -------------
         v125: the real thing, replacing the margin columns. The columns
         solved overlap by decree and broke ASSOCIATION - every label sat
         at the map edge, tied to its country only by a long leader, and
         the grower called it. The cartographic model (Imhof's rules; the
         8-position candidate model in the dynamic-labeling literature)
         places each label ADJACENT to its feature, choosing among ranked
         candidate positions around it and penalizing candidates that
         cover OTHER features. The NP-hard formulations are about
         thousands of labels; at our cap of ten, one greedy pass in
         area order against ranked candidates is the standard answer.

         Candidate ranking: INSIDE beats everything when the name truly
         fits (for an area feature, a label inside the area is the
         strongest association there is), then NE, NW, SE, SW, E, W, N,
         S. A candidate is priced by how much LIT ground of OTHER
         countries it covers - covering your own country is association,
         covering the neighbour is mislabeling, which is exactly how
         v123's first cut put "IVORY COAST" on top of Ghana. Zero-cost
         wins at the best rank; otherwise the cheapest candidate. */
      ...
      /* every lit shape's bbox with its owner. Shape-level, not
         country-level: after a retag a country can light two small
         separated pieces, and the union bbox would wrongly price the
         dark gap between them as occupied ground. */
      var litbb = [];
      Object.keys(onSet).forEach(function(n){
        var b = PATHBB[n]; if (!b) return;
        var h = HIER3[n] || {};
        litbb.push({ bb: b, c: h.country || (h.level === 3 ? n : null) });
      });
      ...
      function litCost(r, own){
        var cost = 0;
        for (var i = 0; i < litbb.length; i++){
          var e = litbb[i];
          if (e.c === own) continue;
          var b = e.bb;
      ...
      /* largest lit area first: the big countries anchor the layout,
         the crowded small ones then fit in around them */
      rows.sort(function(a,b){ return b.area - a.area; });
      rows.forEach(function(r){
        var bb = r.bb, tw = r.tw, own = r.name;
        var cands = [];
        ...
        var bbE = bb[2] + gap, bbW = bb[0] - gap, bbN = bb[1] - gap, bbS = bb[3] + gap;
        [
          { x:bbE, y:bbN + cfs*0.9,    a:"start",  rank:1 },   /* NE */
          { x:bbW, y:bbN + cfs*0.9,    a:"end",    rank:2 },   /* NW */
          { x:bbE, y:bbS - cfs*0.1,    a:"start",  rank:3 },   /* SE */
          { x:bbW, y:bbS - cfs*0.1,    a:"end",    rank:4 },   /* SW */
          { x:bbE, y:r.cy + cfs*0.35,  a:"start",  rank:5 },   /* E  */
          { x:bbW, y:r.cy + cfs*0.35,  a:"end",    rank:6 },   /* W  */
          { x:r.cx, y:bbN - cfs*0.15,  a:"middle", rank:7 },   /* N  */
          { x:r.cx, y:bbS + cfs*0.9,   a:"middle", rank:8 }    /* S  */
        ].forEach(function(c){
        ...
        for (var i = 0; i < cands.length; i++){
          ...
          c.cost = litCost(c.rect, own);
          if (!best || c.cost < best.cost - 1e-9) best = c;
          /* candidates arrive in preference order, so the FIRST clean
             one is the best clean one - stop looking */
        ...
        if (r.ln && !best.inside){
          /* adjacent labels associate by PROXIMITY (Imhof) - a leader on
             a touching label is clutter. Draw one only when the label
             had to settle somewhere away from its own feature. */
          var lx = best.a === "start" ? best.rect[0] + gap*0.4
                 : best.a === "end"   ? best.rect[2] - gap*0.4 : best.x;
          var ly = best.y - cfs*0.3;
          var nx = Math.max(bb[0], Math.min(bb[2], lx));
          var ny = Math.max(bb[1], Math.min(bb[3], ly));
          var far = Math.sqrt((lx-nx)*(lx-nx) + (ly-ny)*(ly-ny)) > gap*2.2;
          r.ln.style.display = far ? "" : "none";
          if (far){
  ...
  function boot(){
    ...
    mounts.forEach(function(mount){
      ...
      mount.setAttribute("data-apsc-version", "card-v125-file-v147");
      if (window.console && console.info){
