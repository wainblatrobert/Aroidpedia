/* AROIDPEDIA footer bundle — built 2026-08-21 from FILE v155 by build-footer-bundle v1.
...
(function(){
  ...
  window.__apFooterBundle = "v155";
  if (window.console && console.info) console.info("[footer bundle] FILE v155 (hosted footer.js)");
...
(function(){
  ...
  function buildMap(data, hits, doubtful, wash, subparent){
    ...
    (function(){
      /* one axis: a context window of width z*F around centre c, kept
         INSIDE [lo,hi]. Prefer staying CENTRED by shrinking (a range
         near the map edge - longiloba reaches the drawn world's east
         rim - would otherwise slide sideways and un-centre, the exact
         bug being fixed); fall back to sliding only when centring
         would cost too much of the zoom-out. */
      function fitAxis(c, z, F, lo, hi){
        var w = Math.min(z * F, hi - lo);
        var wc = Math.min(w, 2 * (hi - c), 2 * (c - lo));
        if (wc >= z * 1.3) w = wc;
        var x = c - w / 2;
        if (x < lo) x = lo;
        if (x + w > hi) x = hi - w;
      ...
      if (Math.min(zoom[2] * 2.5, full[2]) <= zoom[2] + 1e-9){ WORLDVB = full.slice(); return; }
      var cx = zoom[0] + zoom[2] / 2, cy = zoom[1] + zoom[3] / 2;
      var fx = fitAxis(cx, zoom[2], 2.5, full[0], full[0] + full[2]);
      /* keep the framed aspect so nothing distorts; letterboxing is fine */
      var h = Math.min(fx[1] * (zoom[3] / zoom[2]), full[3]);
      var fy = fitAxis(cy, zoom[3], h / zoom[3], full[1], full[1] + full[3]);
      WORLDVB = [fx[0], fy[0], fx[1], fy[1]];
    })();
  ...
  function boot(){
    ...
    mounts.forEach(function(mount){
      ...
      mount.setAttribute("data-apsc-version", "card-v133-file-v155");
      if (window.console && console.info){
