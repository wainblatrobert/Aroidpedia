/* AROIDPEDIA footer bundle — built 2026-08-28 from FILE v183 by build-footer-bundle v1.
...
(function(){
  ...
  var VERSION = "apgm-v19-file-v183";
  ...
  function render(host, genus, shapes, geo, g, hier){
    ...
    ((hier && hier.mapLabels) || []).forEach(function(nm){
      ...
      t.setAttribute("x", ((lb[0] + lb[2]) / 2).toFixed(2));
      t.setAttribute("y", ((lb[1] + lb[3]) / 2).toFixed(2));
      t.setAttribute("font-size", (HOME[2] * 0.016).toFixed(2));
      t.textContent = nm;
    ...
    var hasSubzones = zones.concat(doubt).some(function(z){ return depthOf(z) >= 2; });
    var hasDivisions = zones.concat(doubt).some(function(z){ return depthOf(z) >= 3; });
    ...
    function apply(view){
      ...
      order.forEach(function(z){
        ...
        if (gr && gr.merged){
          var mixv = function(a, b){ return Math.round(b + (a - b) * fo); };
          paths[z].style.stroke = "rgb(" + mixv(175, 19) + "," +
                                  mixv(192, 26) + "," + mixv(144, 21) + ")";
          paths[z].style.strokeOpacity = "1";
        } else {
          paths[z].style.stroke = "";
          paths[z].style.strokeOpacity = "";
        }
      });
      Object.keys(spaths).forEach(function(k){
        var sg = groupMap[k];
        spaths[k].style.fillOpacity = (view === "countries" && sg) ? ramp(sg.count, max) : 0;
...
(function(){
  ...
  function boot(){
    ...
    mounts.forEach(function(mount){
      ...
      mount.setAttribute("data-apsc-version", "card-v148-file-v183");
      if (window.console && console.info){
