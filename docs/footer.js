/* AROIDPEDIA footer bundle — built 2026-09-01 from FILE v209 by build-footer-bundle v1.
...
(function(){
  ...
  /* v79 (8.31.26): FIGURE LINKS WRITTEN BY CODE BLOCKS.
     The genus pages' anatomy plates are  <a class="ap-fig-link" href="...">
     <img></a>  emitted by a code block, and enableForImageBlocks above can
     never reach them for TWO independent reasons: they are not
     .image-block-wrapper, and its handler deliberately bails on
     e.target.closest('a, button'). So every plate on /arum opened in a new
     tab, which is what a reader noticed. Measured before changing anything:
     /arum carries 0 .image-block-wrapper and 4 a.ap-fig-link.

     Same overlay, same CSS, one more binding. The href STAYS, so with JS
     dead the plate still opens in a tab, and modified clicks are left alone
     so ctrl/cmd/middle-click still get a real tab.

     Note the root.matches branch: the MutationObserver below hands us the
     ADDED NODE, and querySelectorAll would miss it when the node IS the
     link rather than an ancestor of one. */
  function enableForFigureLinks(root=document){
    const links = (root.matches && root.matches('a.ap-fig-link[href]'))
      ? [root]
      : root.querySelectorAll('a.ap-fig-link[href]');
    links.forEach(a => {
      if (a.dataset.apLightboxBound === '1') return;
      a.dataset.apLightboxBound = '1';

      a.addEventListener('click', (e) => {
        // leave modified clicks alone - they must still open a real tab
        ...
        e.preventDefault();
        e.stopPropagation();
        openOverlay(href, img ? img.getAttribute('alt') : '');
      });
    });
  }
  ...
  document.addEventListener('DOMContentLoaded', () => {
    enableForImageBlocks();
    enableForFigureLinks();
  });
  ...
  const mo = new MutationObserver((mutations) => {
    for (const m of mutations) {
      m.addedNodes && m.addedNodes.forEach(node => {
        if (node.nodeType === 1) { enableForImageBlocks(node); enableForFigureLinks(node); }
      });
    }
  });
...
(function(){
  ...
  var HIER_URL = "https://wainblatrobert.github.io/Aroidpedia/geo-hierarchy.json";
  ...
  var VERSION = "apgm-v42-file-v207";
  var NS = "http://www.w3.org/2000/svg";

  /* ==== F6 (8.28.26) — THE SHARED-ARC TOPOLOGY LOADER ==============
     shapes-topo.json stores every shared border ONCE as a quantized
     delta-encoded arc (built by build-shapes-topo.mjs over the shape
     builder's UNSIMPLIFIED rings), so two neighbours cannot disagree
     about their common edge at any tolerance — the overlaps and the
     "so jaggedy" zooming were both symptoms of per-polygon
     simplification, which this retires. The decoder below rebuilds the
     EXACT {shapes:{tag:'M…Z'}, order, area, continent, dots, viewBox,
     viewBoxAll, borders} contract every consumer already speaks, so
     nothing downstream changes. shapes-hd.json STAYS PUBLISHED and is
     the fallback on any failure: worst case, the site looks exactly as
     it did yesterday. Shared window-level: the genus map and the
     species card both call this, one fetch between them. */
  window.__apShapes = window.__apShapes || function(){
    if (window.__apShapesP) return window.__apShapesP;
    var TOPO = "https://wainblatrobert.github.io/Aroidpedia/shapes-topo.json";
    var HD1 = "https://wainblatrobert.github.io/Aroidpedia/shapes-hd.json";
    var HD2 = "https://raw.githubusercontent.com/wainblatrobert/Aroidpedia/main/docs/shapes-hd.json";
    function dec(T){
      var sx = T.transform.scale[0], sy = T.transform.scale[1];
      var tx = T.transform.translate[0], ty = T.transform.translate[1];
      var arcs = T.arcs.map(function(a){
        var x = 0, y = 0;
        return a.map(function(p){ x += p[0]; y += p[1]; return [x * sx + tx, y * sy + ty]; });
      });
      var shapes = {};
      Object.keys(T.places).forEach(function(tag){
        var d = "";
        T.places[tag].forEach(function(ring){
          var pts = [];
      ...
      /* v31: REGION BORDERS - arcs separating two different regions,
         shipped as {region: [arc indexes]} (build-shapes-topo v3).
         Open polylines on purpose: closing them would draw chords. */
      var rb = {}, cb = {};
      [["rborders", rb], ["cborders", cb]].forEach(function(pair){
      var SRC0 = T[pair[0]] || {}, DST0 = pair[1];
      Object.keys(SRC0).forEach(function(R){
        var segs = [];
        SRC0[R].forEach(function(e){
          /* v32: an entry is [arcIdx, ...adjacent zone tags] - the
             genus map draws a segment only where a side PAINTS. */
          var a = arcs[e[0]];
          if (!a || a.length < 2) return;
          segs.push({ z: e.slice(1), d: "M" + a.map(function(p){
            return p[0].toFixed(2) + " " + (-p[1]).toFixed(2);
          }).join("L") });
        });
        if (segs.length) DST0[R] = segs;
      });
      });
      return { shapes: shapes, rborders: rb, cborders: cb,
               order: T.order, area: T.area,
               continent: T.continent, dots: T.dots, viewBox: T.viewBox,
               viewBoxAll: T.viewBoxAll, borders: T.borders, topo: 1 };
    }
    window.__apShapesP = fetch(TOPO, {mode: "cors"})
      .then(function(r){ if (!r.ok) throw 0; return r.json(); })
      .then(dec)
      .catch(function(){
        return fetch(HD1, {mode: "cors"})
          .then(function(r){ if (!r.ok) throw 0; return r.json(); });
      })
      .catch(function(){
        return fetch(HD2, {mode: "cors"}).then(function(r){ return r.json(); });
      });
    return window.__apShapesP;
  };
  ...
  function feeds(){
    ...
    if (!fetched){
      fetched = Promise.all([
        window.__apShapes(),   /* F6: topology first, shapes-hd fallback */
        fetch(GEO_URL, {mode:"cors"}).then(function(r){ if (!r.ok) throw new Error(r.status); return r.json(); }),
        /* v9: the hierarchy, for a Countries view that actually groups
           by country. Optional on its own: if it fails, Countries
           falls back to the zones grouping it always was. */
        fetch(HIER_URL, {mode:"cors"}).then(function(r){ if (!r.ok) throw new Error(r.status); return r.json(); }).catch(function(){ return null; })
  ...
  function render(host, genus, shapes, geo, g, hier){
    var zones = [], missing = [];
    ...
    /* ==== v17 — THE POLITICAL PROJECTION (hier 1.29.0) ================
       Grower-approved redesign (MAP HIERARCHY — REVIEW & PROPOSAL
       8.28.26): the views run on continent > region > country > state >
       division, while WGSRPD stays the recording vocabulary. A place
       with geo:1 (Borneo, China Southeast, Java, old-Assam…) is a
       GEOGRAPHIC unit: taggable, drawn, residual-carrying — never a row
       in a political view. Its children promote to the nearest non-geo
       ancestor. Chains come from the HIERARCHY feed, not from SUB —
       hier parentage is the superset (SUB is only the tag-refined
       places) and it is the same authored l4 tables underneath. */
    var HP = (hier && hier.places) || {};
    function dispName(z){ return (HP[z] && HP[z].label) || z; }
    function admDepth(z){
      /* -1 geographic (never a row) · 0 country-anchor/container ·
         1 state · 2 division. Geo ancestors are transparent. */
      var v = HP[z] || {};
      if (v.geo) return -1;
      var C = v.country;
      if (!C || z === C) return 0;
      var d = 1, cur = v.parent, gd = 0;
      while (cur && cur !== C && gd++ < 8){
        var w = HP[cur] || {};
        if (!w.geo) d++;
        cur = w.parent;
      }
    ...
    function setVB(){
      ...
      /* v27: the world-context hairline is 0.15-degree-simplified - past
         ~2.5x zoom its long straight segments diverge from the detailed
         coastlines and read as stray chords (grower screenshot,
         Divisions over Cambodia). Context is an overview idea; it fades
         where it stops being true. */
      svg.classList.toggle("apgm--zoomed", CUR[2] < HOME[2] / 2.5);
    ...
    /* v11: double-click zooms in toward the pointer (grower request,
       same gesture as the journal map). */
    box.addEventListener("dblclick", function(ev){
      ev.preventDefault();
      var p = userPoint(ev.clientX, ev.clientY);
      if (!p) return;
      zoomAt(p.x, p.y, 1.4 * 1.4);
    });
    ...
    svg.addEventListener("pointermove", function(ev){
      ...
      if (Math.abs(ev.clientX - pan.x) + Math.abs(ev.clientY - pan.y) > 4){
        dragged = true;
        closePanel();   /* v9: a drag is a navigation, not a selection */
    ...
    var zoomRow = el("div", "apgm__zoom-row");
    ...
    zoomUI.appendChild(zoomRow);
    /* v40: CLIMATE sits under the zoom pills and spans their width. It
       swaps the species heatmap for the climate bands WITHOUT changing
       the view, so whichever geo pill is selected keeps its rows
       hoverable underneath (grower). */
    var climBtn = el("button", "apgm__climbtn", "CLIMATE");
    climBtn.type = "button";
    climBtn.setAttribute("aria-pressed", "false");
    climBtn.setAttribute("aria-label", "Colour the range by climate zone");
    climBtn.addEventListener("click", function(){
      climOn = !climOn;
      climBtn.setAttribute("aria-pressed", climOn ? "true" : "false");
      climBtn.classList.toggle("is-on", climOn);
      setHot(null);
      apply(currentView);
    });
    zoomUI.appendChild(climBtn);
    ...
    function clearSel(){
      ...
      order.forEach(function(z){
        ...
        paths[z].classList.remove("is-ctx1");
        paths[z].classList.remove("is-ctx0");
      });
      Object.keys(dpaths).forEach(function(z){ dpaths[z].classList.remove("is-sel"); });
      Object.keys(spaths).forEach(function(k){ spaths[k].classList.remove("is-sel"); });
      if (searchUI){
    ...
    function selectEp(ep){
      ...
      order.forEach(function(z){
        ...
        paths[z].classList.remove("is-dsel");
        /* v15: the card's depth ladder in the selection too - a
           suppressed container used to fall into the .06 dim with
           everything unrelated. Middle shelf .28, top of the chain
           .07 with its outline, exactly the species-card treatment. */
        var d = (suppress[z] && natSet[z]) ? depthOf(z) : -1;
        paths[z].classList.toggle("is-ctx1", d >= 1);
        paths[z].classList.toggle("is-ctx0", d === 0);
        if (on) nNat++;
      });
      Object.keys(spaths).forEach(function(k){
        var z = SPLIT[k].zone;
        spaths[k].classList.toggle("is-sel", !!(natSet[z] && !suppress[z]) || !!natSet[k]);
      });
    ...
    /* apgm v8 — WHICH SOURCE THE NUMBER CAME FROM. Since genus-geo
       1.18.0, speciesTotal is the LARGER of POWO's accepted species and
       the journal's own species posts (speciesTotalPowo/speciesJournal
       carry both). Captioning a journal-sourced 118 "per POWO/WCVP"
       misattributes it — POWO says 91 for Alocasia. An old feed lacks
       speciesTotalPowo, the test is false, and the old line stands. */
    wrap.appendChild(el("div", "apgm__note",
      ((g.speciesJournal || 0) > (g.speciesTotalPowo || Infinity)
        ? g.speciesTotal + " species · per POWO/WCVP (" + g.speciesTotalPowo +
          ") + the journal archive · "
    ...
    /* v17: CARTOGRAPHIC LABELS (grower: "Borneo as a generic label on
       the map ... whenever a zone of any level is present in view").
       Island names from hier.mapLabels, drawn as quiet uppercase text at
       the unit's own shape centre, at every view, never interactive.
       Appended AFTER the zone paths below (see maplabelHost). */
    var maplabelHost = svgEl("g");

    /* v14: THE SHELF A SUBUNIT SITS ON. The feed files everything
       below a zone as a "subunit", but Borneo carries two depths -
       Sarawak (one step down) and Sarawak's divisions (two). Subzones
       now draws the first shelf only; a new Divisions view draws both.
       Depth is the length of the SUB chain, never a feed field. */
    function depthOf(z){
      /* v17: POLITICAL depth (was the SUB chain). 0 = container ground
         (a country anchor or a geo unit), 1 = state, 2 = division.
         Peninsular Malaysia, a plain zone until now, correctly becomes
         container ground in the States view — its 13 states are the
         rows. */
      var d = admDepth(z);
      return d <= 0 ? 0 : d;
    }
    function depthClass(p, z){
      var d = depthOf(z);
      if (d) p.classList.add(d >= 2 ? "apgm-d2" : "apgm-d1");
      if (d >= 3) p.classList.add("apgm-d3");   /* v18: third shelf */
    }
    ...
    order.forEach(function(n){
      ...
      if (SUB[n]){ p.classList.add("apgm-sub"); depthClass(p, n); }
    ...
    doubt.forEach(function(n){
      ...
      if (SUB[n]){ p.classList.add("apgm-sub"); depthClass(p, n); }   /* v7: Subzones-view only */
      p.setAttribute("d", shapes.shapes[n]);
      p.setAttribute("data-zone", n);
      svg.appendChild(p);
      dpaths[n] = p;
    });
    /* v7 -> v17: which ancestors have refined data below them in THIS
       genus — now the full political chain, so a geo unit (China
       Southeast) banding over Fujian's data works exactly like Borneo
       over Sarawak's. Only ancestors with a shape can band. */
    var PARENTS = {};
    zones.concat(doubt).forEach(function(z){
      if (admDepth(z) < 1) return;
      chainOf(z).forEach(function(a){
        if (shapes.shapes[a]) PARENTS[a] = 1;
      });
    });
    /* v11: GHOST SUBUNITS. A parent with subunit data drew only the
       subunits this genus is tagged in, so an untagged sibling (North
       Kalimantan, West Sulawesi) was a hole where the parent showed
       through - hover read "Kalimantan" and lit the whole block
       (grower, twice). Every hierarchy child of a drawn parent that
       has a shape now draws as an unlit outline that names itself;
       it takes no fill, no count and no panel. */
    var ghosts = {};
    var ghostPaths = {};                       /* v18.1: name -> element */
    /* v17: HP is declared with the projection helpers above. The gate
       widens: a ghost draws under ANY drawn ground, not only under a
       parent that already has data — so hovering inside Thailand names
       the province even before a single record is placed there
       (grower: "all countries should break down"). */
    Object.keys(HP).forEach(function(k){
      var par = HP[k].parent;
      var ok = par && (PARENTS[par] || paths[par] || dpaths[par]);
      if (!ok || paths[k] || dpaths[k] || !shapes.shapes[k]) return;
      var p = svgEl("path");
      p.setAttribute("class", "apgm-zone apgm-sub apgm-ghost");
      /* a ghost's depth is its parent's plus one (it has no SUB entry) */
      var gpd = depthOf(par) + 1;
      p.classList.add(gpd >= 2 ? "apgm-d2" : "apgm-d1");
      if (gpd >= 3) p.classList.add("apgm-d3");   /* v18 */
      p.setAttribute("d", shapes.shapes[k]);
      p.setAttribute("data-zone", k);
      svg.appendChild(p);
      ghosts[k] = par;
      ghostPaths[k] = p;                       /* v18.1 */
    });
    /* v13: A MULTI-COUNTRY ZONE SPLITS IN THE COUNTRIES VIEW. Borneo
       has no single country, so it stood as "Borneo" between Indonesia
       and Malaysia (grower: "borneo is a separate country when it should
       show as part of indonesia if hovering over the south and malaysia
       over the north"). The zone's polygon now joins the country holding
       the most of it (by shape area), and the OTHER countries' children
       draw on top as splits - so there is never a hole, even where the
       children do not tile the parent (Lesser Sunda). The zone's species
       count toward EVERY country on it: a plant known only as "Borneo"
       is in Indonesia or Malaysia, and the honest answer to "is it in
       Malaysia" is not "no". A zone whose children all name ONE country
       (the Brazil and Mexico units, Rwanda, Eritrea) simply files there. */
    var MULTI = {}, spaths = {}, SPLIT = {};
    (function(){
      if (!hier || !hier.places) return;
      var H = hier.places, area = shapes.area || {};
      zones.forEach(function(z){
        if (SUB[z] || (H[z] && H[z].country)) return;
        var byC = {};
        Object.keys(H).forEach(function(k){
          if (H[k].parent !== z || !H[k].country) return;
          (byC[H[k].country] = byC[H[k].country] || []).push(k);
        });
        var cs = Object.keys(byC);
        if (!cs.length) return;
        var main = cs[0], best = -1;
        cs.forEach(function(c){
          var a = 0; byC[c].forEach(function(k){ a += area[k] || 0; });
          if (a > best){ best = a; main = c; }
        });
        MULTI[z] = { main: main, countries: cs };
        if (cs.length < 2) return;
        cs.forEach(function(c){
          if (c === main) return;
          byC[c].forEach(function(k){
            if (!shapes.shapes[k]) return;
            var p = svgEl("path");
            p.setAttribute("class", "apgm-zone apgm-split");
            p.setAttribute("d", shapes.shapes[k]);
            p.setAttribute("data-zone", k);
            svg.appendChild(p);
            spaths[k] = p;
            SPLIT[k] = { country: c, zone: z };
          });
        });
      });
    })();
    /* v22: COUNTRY GHOSTS. The child-ghost gate above only widens
       DOWNWARD, so a country with no country-grade tag (Australia -
       only Queensland and New South Wales carry Alocasia) had no shape
       to complete it in the political views. Every country implied by
       this genus's zones now draws as an inert ghost; apply() lights
       it at the group fill and the member zones dissolve into it. */
    (function(){
      var want = {};
      zones.forEach(function(z){
        var hc = HP[z];
        var c = (hc && hc.country) || (MULTI[z] && MULTI[z].main) || null;
        if (c && c !== z) want[c] = 1;
        if (MULTI[z] && MULTI[z].countries){
          MULTI[z].countries.forEach(function(cc){ if (cc !== z) want[cc] = 1; });
        }
      });
      Object.keys(want).forEach(function(c){
        if (paths[c] || dpaths[c] || ghostPaths[c] || !shapes.shapes[c]) return;
        var p = svgEl("path");
        p.setAttribute("class", "apgm-zone apgm-ghost apgm-cg");
        p.setAttribute("d", shapes.shapes[c]);
        p.setAttribute("data-zone", c);
        /* under everything: the borders path first, then ground */
        svg.insertBefore(p, svg.firstChild ? svg.firstChild.nextSibling : null);
        ghosts[c] = c;          /* truthy for the hover gate; setGhost is
                                   unreachable while it is inert */
        ghostPaths[c] = p;
      });
    })();
    ...
    /* v27: GHOSTS STACK BY DEPTH - hovering inside a division ghost
       ...
       Country ghosts (apgm-cg) stay at the bottom where they belong. */
    Object.keys(ghostPaths)
      .filter(function(k){ return !ghostPaths[k].classList.contains("apgm-cg"); })
      .map(function(k){
        var el = ghostPaths[k];
        return [k, el.classList.contains("apgm-d3") ? 3
                 : el.classList.contains("apgm-d2") ? 2 : 1];
      })
      .sort(function(a, b){ return a[1] - b[1]; })
      .forEach(function(pair){ svg.appendChild(ghostPaths[pair[0]]); });
    /* v25: STRUCTURAL CONTAINMENT (the journal's v20.18 law). A place
       is a container in a subview when the view DRAWS something
       inside it - ghosts count, data or not. Without this the
       Subzones and Divisions ramps echoed the zone view: a zone with
       only ghost children stayed a bright leaf at its full family
       count, so nothing visually changed shelf to shelf (grower:
       "heatmap does not work at lower levels"). */
    var HASKID = {};
    Object.keys(ghosts).forEach(function(k){
      var p2 = ghosts[k];
      if (p2 && p2 !== k) HASKID[p2] = 1;
    });
    Object.keys(SUB).forEach(function(k){
      if (paths[k] || dpaths[k] || ghostPaths[k]) HASKID[SUB[k]] = 1;
    });
    /* v20 (#9): each label gets an authored OFFSHORE direction (unit-ish
       vector, y negative = north in this flipped frame) and a leader
       line back to its island's edge. */
    /* v21: Borneo's old [0.55,-1] (north-east) landed the text on the
       Philippines; the grower wants it in the sea between Borneo and
       Vietnam - north-west. */
    var LABEL_DIR = { "Borneo": [-0.66, -0.75, 0.72], "Sumatra": [-1, 0.55],
                      "Java": [-0.35, 1], "Sulawesi": [0.58, 0.81, 0.77],
                      "Luzon": [1, -0.35], "Mindanao": [1, 0.4],
                      "New Guinea": [0.75, -0.85] };
    /* v33: LABELS SIZE TO THE GEOGRAPHY, NOT THE VIEWPORT. Everything
       below scaled with HOME[2], tuned on Asian genera (~95 deg wide).
       Amorphophallus spans Africa + Asia (~185 deg), so its labels
       doubled in geographic size over islands half the screen size
       (grower: "labels are much too big in asia"). Clamped, an Asian
    ...
    ((hier && hier.mapLabels) || []).forEach(function(nm){
      ...
      var hw = (lb[2] - lb[0]) / 2, hh = (lb[3] - lb[1]) / 2;
      var dir = LABEL_DIR[nm] || [1, -0.4];
      var mag = Math.sqrt(dir[0] * dir[0] + dir[1] * dir[1]) || 1;
      var ux = dir[0] / mag, uy = dir[1] / mag;
      /* v22: an optional third element scales the offshore DISTANCE -
         direction alone rides the bbox rim, and Borneo's rim point sat
         on Vietnam's coast (grower: "slightly lower and to the right"
         means INSIDE the rim, which only a distance knob can do). */
      var lsc = dir[2] || 1;
      var edgeX = cx + ux * hw * 0.92, edgeY = cy + uy * hh * 0.92;
      var pad = LBASE * 0.02;
      var lx = cx + ux * (hw + pad) * 1.06 * lsc, ly = cy + uy * (hh + pad) * 1.06 * lsc;

      var line = svgEl("line");
      line.setAttribute("class", "apgm-maplead");
      line.setAttribute("x1", edgeX.toFixed(2)); line.setAttribute("y1", edgeY.toFixed(2));
      line.setAttribute("x2", lx.toFixed(2)); line.setAttribute("y2", ly.toFixed(2));
      maplabelHost.appendChild(line);
      /* v26: the leader's inner end MARCHES TO THE COAST. The bbox
         ellipse rim is open sea for a sprawling island (Borneo, the
         grower's worst offender). isPointInFill yields nothing before
         layout - the covProbe lesson - so the march retries on frames
         until the geometry answers, and the rim stands if it never
         does. */
      (function(line2, nm2, lx2, ly2, cx2, cy2){
        var tries = 0;
        var march = function(){
          var probeEl = paths[nm2] || dpaths[nm2] || ghostPaths[nm2] || null;
          if (probeEl && probeEl.isPointInFill){
            try {
              var ptp = svg.createSVGPoint();
              for (var tt = 0.02; tt <= 1; tt += 0.02){
                ptp.x = lx2 + (cx2 - lx2) * tt;
                ptp.y = ly2 + (cy2 - ly2) * tt;
                if (probeEl.isPointInFill(ptp)){
                  line2.setAttribute("x1", ptp.x.toFixed(2));
                  line2.setAttribute("y1", ptp.y.toFixed(2));
                  return;
                }
              }
            } catch (e) {}
          }
          if (++tries < 40) requestAnimationFrame(march);
        };
        requestAnimationFrame(march);
      })(line, nm, lx, ly, cx, cy);
      var t = svgEl("text");
      t.setAttribute("class", "apgm-maplabel");
      t.setAttribute("x", (lx + ux * pad * 0.4).toFixed(2));
      t.setAttribute("y", (ly + uy * pad * 0.4 + LBASE * 0.005).toFixed(2));
      t.setAttribute("text-anchor", ux > 0.25 ? "start" : ux < -0.25 ? "end" : "middle");
      t.setAttribute("font-size", (LBASE * 0.014).toFixed(2));
      t.textContent = nm;
      maplabelHost.appendChild(t);
    });
    /* v31: REGION BORDERS (grower: "add borders in regions, to better
       differentiate the regions visually - the heatmap colors are
       pretty similar"). One stroke-only path per region, lit only for
       regions the genus paints, only in the Regions view. */
    /* v42: the same layer now serves CONTINENTS as well. Continents had
       no boundary at all - its seam stroke is compFill(), the shape's
       own fill, which is a crack-filler and blends whenever two groups
       land on similar values (grower). Keyed "view|group" so one host
       carries both sets and only the current view's are lit. */
    var gbSrc = { regions: shapes.rborders || {}, continents: shapes.cborders || {} };
    var gbPaths = {}, gbBuilt = {};
    (function(){
      var gbHost = svgEl("g");
      Object.keys(gbSrc).forEach(function(vw){
        Object.keys(gbSrc[vw]).forEach(function(g){
          var p = svgEl("path");
          p.setAttribute("class", "apgm-rb");
          /* d is assembled on the first apply of that view, once the
             genus's painted footprint is known. */
          gbHost.appendChild(p);
          gbPaths[vw + "|" + g] = p;
        });
      });
      svg.appendChild(gbHost);
    })();
    /* ==== v38 - THE CLIMATE BANDS LAYER ==========================
       World Terrestrial Ecosystems temperature x moisture domains,
       rasterised to a palette-indexed PNG in plain lon/-lat (Sayre et
       al. 2020 - the same product the climate pipeline measures every
       place against). Because the map's user space IS lon/-lat the
       image drops in at x -180..180, y -90..90 with NO reprojection,
       clipped to the range polygons already on screen. Loaded lazily:
       a page that never opens the Climate view never fetches it.
       Range geos only, by design - a political rollup would have to
       average a climate across a whole country, which means nothing. */
    var CLIM_PNG  = "https://wainblatrobert.github.io/Aroidpedia/climate-zones.png";
    var CLIM_JSON = "https://wainblatrobert.github.io/Aroidpedia/climate-zones.json";
    var climHost = svgEl("g");
    svg.appendChild(climHost);
    var climMeta = null, climByHex = null, climData = null;
    var climCW = 2160, climCH = 1080;
    var climP = null, climPresent = null, climFailed = 0;
    var climCp = null, climEdgePath = null, climHotPath = null, climClipView = null;
    climHost.style.display = "none";
    var climKey = function(r, g, b){
      var h = function(v){ v = v.toString(16); return v.length < 2 ? "0" + v : v; };
      return "#" + h(r) + h(g) + h(b);
    };

    function climLoad(){
      if (climP) return climP;
      climP = Promise.all([
        fetch(CLIM_JSON, { mode: "cors" }).then(function(r){ return r.json(); }),
        new Promise(function(res, rej){
          var im = new Image();
          im.crossOrigin = "anonymous";      /* docs/ serves ACAO * */
        ...
        climMeta = a[0];
        climByHex = {};
        (climMeta.zones || []).forEach(function(z, i){
          if (z && z.hex) climByHex[z.hex.toLowerCase()] = i;
        });
        /* ONE downsampled copy for the readout, nearest neighbour:
           smoothing would blend two zones into a colour that is
           neither, and the lookup is by exact palette colour. */
        var cv = document.createElement("canvas");
        cv.width = climCW; cv.height = climCH;
        var g2 = cv.getContext("2d", { willReadFrequently: true });
        g2.imageSmoothingEnabled = false;
        g2.drawImage(a[1], 0, 0, climCW, climCH);
        climData = g2.getImageData(0, 0, climCW, climCH).data;
        return a[1];
      }).catch(function(e){
        climFailed = 1;
        if (window.console) console.warn("[apgm] climate layer unavailable", e);
        return null;
      });
      return climP;
    }

    /* which zones the range actually touches - paint the world raster,
       punch the range out of it, count what survives. The legend then
       lists what is on screen and nothing else. */
    function climScan(im, ds){
      try {
        var cv = document.createElement("canvas");
        cv.width = 720; cv.height = 360;
        var g2 = cv.getContext("2d", { willReadFrequently: true });
        g2.imageSmoothingEnabled = false;
        g2.drawImage(im, 0, 0, 720, 360);
        g2.globalCompositeOperation = "destination-in";
        g2.setTransform(2, 0, 0, 2, 360, 180);   /* lon,-lat -> canvas px */
        /* ONE path, one fill: destination-in does not accumulate - a
           second fill would intersect with the first, and the range
           would come back empty. */
        var all = new Path2D();
        ds.forEach(function(d){ try { all.addPath(new Path2D(d)); } catch (e) {} });
        g2.fill(all);
        g2.setTransform(1, 0, 0, 1, 0, 0);
        var px = g2.getImageData(0, 0, 720, 360).data, seen = {}, tot = 0;
        for (var i = 0; i < px.length; i += 4){
          if (px[i + 3] < 128) continue;
          var zi = climByHex[climKey(px[i], px[i + 1], px[i + 2])];
          if (zi){ seen[zi] = (seen[zi] || 0) + 1; tot++; }
        }
        /* v39: the key SUMMARISES. A range spanning the Sahara to
           Scandinavia to Tibet touches seventeen zones, several of them
           a handful of cells the gap fill left behind (Boreal Desert,
           Polar Desert - which carry no measured range either). Under
           half a percent of the painted range is noise; twelve chips is
           the most a key can be read at a glance. */
        return Object.keys(seen).map(Number)
          .filter(function(z){ return seen[z] / Math.max(1, tot) >= 0.005; })
          .sort(function(a, b){ return seen[b] - seen[a]; })
          .slice(0, 12);
      } catch (e) { return []; }
    }

    /* v41: WHAT THE VIEW PAINTS IS WHAT THE CLIMATE COVERS. The clip
       used to be built once from the recorded zones, so on Alocasia
       Regions the geography filled all of China while the bands stopped
       at the recorded provinces (grower). In a political view the unit
       IS the political boundary, so the clip is rebuilt per view from
       the shapes actually carrying fill - lit country ghosts included. */
    function climShapes(){
      var ds = [];
      order.forEach(function(z){
        var el = paths[z];
        if (!el) return;
        if (!(parseFloat(el.style.fillOpacity || 0) > 0.05)) return;
        var d = el.getAttribute("d"); if (d) ds.push(d);
      });
      Object.keys(ghostPaths).forEach(function(k){
        var el = ghostPaths[k];
        if (!el.classList.contains("apgm-famrow")) return;
        var d = el.getAttribute("d"); if (d) ds.push(d);
      });
      return ds;
    }
    function climBuild(im){
      if (!im) return;
      if (!climCp){
        var cid = "apgm-climclip-" + Math.round(HOME[2] * 100) + "-" + order.length;
        var defs = svgEl("defs");
        climCp = svgEl("clipPath"); climCp.setAttribute("id", cid);
        defs.appendChild(climCp); climHost.appendChild(defs);
        var img = svgEl("image");
        img.setAttribute("href", CLIM_PNG);
        img.setAttributeNS("http://www.w3.org/1999/xlink", "href", CLIM_PNG);
        ...
        img.setAttribute("class", "apgm-clim-img");
        climHost.appendChild(img);
        /* the world hairline on top, so a reader can place themselves */
        var bord = svg.querySelector(".apgm-borders");
        if (bord){
          var cl = bord.cloneNode(false);
          cl.setAttribute("class", "apgm-clim-borders");
          cl.setAttribute("clip-path", "url(#" + cid + ")");
          climHost.appendChild(cl);
        }
        /* v41: and the VIEW's own geographies, stroke only - without
           them the bands swallow every boundary and there is nothing
           to hover against (grower). */
        climEdgePath = svgEl("path");
        climEdgePath.setAttribute("class", "apgm-clim-edge");
        climHost.appendChild(climEdgePath);
        climHotPath = svgEl("path");
        climHotPath.setAttribute("class", "apgm-clim-hot");
        climHost.appendChild(climHotPath);
      }
      if (climClipView === currentView) return;
      climClipView = currentView;
      var ds = climShapes();
      while (climCp.firstChild) climCp.removeChild(climCp.firstChild);
      ds.forEach(function(d){ var q = svgEl("path"); q.setAttribute("d", d); climCp.appendChild(q); });
      climEdgePath.setAttribute("d", ds.join(""));
      climPresent = climScan(im, ds);
    }
    /* the hovered row, outlined over the bands */
    function climHotEdge(){
      if (!climHotPath) return;
      if (!climOn){ climHotPath.setAttribute("d", ""); return; }
      var d = "";
      order.forEach(function(z){
        var el = paths[z];
        if (el && el.classList.contains("is-hot")) d += el.getAttribute("d") || "";
      });
      Object.keys(ghostPaths).forEach(function(k){
        var el = ghostPaths[k];
        if (el.classList.contains("is-hot")) d += el.getAttribute("d") || "";
      });
      climHotPath.setAttribute("d", d);
    }

    /* the zone under the cursor - the PIXEL, not the polygon average */
    function climAt(ev){
      if (!climData) return null;
      var pt;
      try {
        pt = svg.createSVGPoint(); pt.x = ev.clientX; pt.y = ev.clientY;
        pt = pt.matrixTransform(svg.getScreenCTM().inverse());
      } catch (e) { return null; }
      var cx = Math.floor((pt.x + 180) / 360 * climCW);
      var cy = Math.floor((pt.y + 90) / 180 * climCH);
      if (cx < 0 || cy < 0 || cx >= climCW || cy >= climCH) return null;
      var i = (cy * climCW + cx) * 4;
      if (climData[i + 3] < 128) return null;
      var zi = climByHex[climKey(climData[i], climData[i + 1], climData[i + 2])];
      return zi ? climMeta.zones[zi] : null;
    }
    /* v40: the geo readout is untouched - setHot still names the row
       under the cursor - and the band is APPENDED to it. climTail runs
       from setHot as well as from mousemove because the browser fires
       mouseover AFTER mousemove when the pointer enters a new shape,
       and setHot clears the readout: appending only on mousemove left
       the band missing until the pointer moved a second time. */
    var climLastEv = null;
    function climTail(){
      var old = hover.querySelector(".apgm__hov-clim");
      if (old) old.parentNode.removeChild(old);
      if (!climOn || !climLastEv || !hover.hasAttribute("data-on")) return;
      var t = climLastEv.target;
      var z = t && t.getAttribute && t.getAttribute("data-zone");
      var zone = (z && paths[z]) ? climAt(climLastEv) : null;
      if (!zone) return;
      var sp = document.createElement("span");
      sp.className = "apgm__hov-clim";
      sp.textContent = " · " + zone.name +
        (zone.t ? " " + zone.t[0] + " to " + zone.t[1] + " °C" : "") +
        (zone.rh ? " · " + zone.rh[0] + "–" + zone.rh[1] + "%" : "");
      hover.appendChild(sp);
    }
    ...
    svg.appendChild(maplabelHost);
    var countryZones = null;   /* v13: country -> zones counted there */
    /* v14: a first-shelf subunit drawn WHOLE carries its family - the
       distinct species recorded in it or in any of its divisions */
    var FAM = null;
    function families(){
      if (FAM) return FAM;
      FAM = {};
      var inv = zoneSpecies();
      Object.keys(inv).forEach(function(z){
        var seen = FAM[z] = FAM[z] || {};
        inv[z].forEach(function(ep){ seen[ep] = 1; });
        chainOf(z).forEach(function(p){
          var sp = FAM[p] = FAM[p] || {};
          inv[z].forEach(function(ep){ sp[ep] = 1; });
        });
      });
      Object.keys(FAM).forEach(function(z){ FAM[z] = Object.keys(FAM[z]).length; });
      return FAM;
    }
    /* v20 (#1/#5): a zone whose ground is ALREADY PAINTED by its drawn
       country contributes its counts and nothing else — India's shape +
       WGSRPD Assam + East Himalaya all at fill .8 stacked into darker
       blobs that vanished on hover (the fill capped at 1). Same story
       inside Borneo, where the island, the country shapes and the state
       shapes piled up. */
    /* v21: COVERED MEANS PAINTED-AND-CONTAINING, NOT MERELY PRESENT.
       Two failures of the v20 existence test, both from the grower's
       Alocasia screenshots ("range - hover - geographies appear"):
       - CIRCULAR SUPPRESSION. Borneo was covered because Brunei's path
         exists - but Brunei is fill-0 in the flat views as Borneo's
         own child, so NOBODY painted the island until hover.
       - CHAIN TRUTH vs GROUND TRUTH. The Andamans were covered because
         India is their chain-country, but India's polygon does not
         contain them - their ground simply vanished.
       A country covers a unit only when it will PAINT in the flat
       views (present, and not itself a child of a geo unit) and its
       shape geometrically CONTAINS the unit's representative point.
       Both inputs are static per genus, so the verdicts cache. */
    var covPt = {}, covCache = {};
    /* v35: did THIS pass's probes have geometry to work with? The first
       apply() runs before layout and every verdict silently came back
       "not covered" (see the re-apply at the end of apply). */
    var covOK = 0, covFail = 0, covRetry = 0;
    var covGhostLit = {};   /* v22: country ghosts painting this apply */
    /* a handful of points ON the unit's own ground - a bbox centre
       alone misfires on the wrap-around units (old-Assam curls around
       Bangladesh, West Himalaya is a thin arc whose centre sits in
       Tibet), which double-painted the Himalayan strip .8 over India's
       .8. 4x4 grid over the bbox, kept only where the unit itself
       fills, capped at 8. */
    function covProbe(z){
      /* v22: cache SUCCESS only - the first apply can run before
         layout, when getBBox yields nothing, and a cached null froze
         every verdict at "unprobeable" for the life of the page. */
      if (covPt[z]) return covPt[z];
      var el = paths[z] || dpaths[z];
      covPt[z] = null;
      if (el && el.getBBox && el.isPointInFill){
        try {
          var bb = el.getBBox();
          var pts = [];
          for (var iy = 0; iy < 4 && pts.length < 8; iy++){
            for (var ix = 0; ix < 4 && pts.length < 8; ix++){
              var pt = svg.createSVGPoint();
              pt.x = bb.x + bb.width * (ix + 0.5) / 4;
              pt.y = bb.y + bb.height * (iy + 0.5) / 4;
              if (el.isPointInFill(pt)) pts.push(pt);
            }
          }
          covPt[z] = pts.length ? pts : null;
        } catch (e) {}
        if (covPt[z]) covOK = 1; else covFail = 1;   /* v35 */
      }
      return covPt[z];
    }
    /* v23: UNION-majority. covPaintsEl answers "which element paints
       for country c"; coveredBy then asks whether most of the unit's
       probe points fall inside ANY of them TOGETHER. The old test
       demanded a majority inside ONE country, so East Himalaya -
       split between India and Bhutan - escaped, painted .8 over
       India's .8, and Arunachal+Sikkim glowed. */
    function covPaintsEl(c, z){
      if (!c || c === z) return null;
      var el = paths[c] || dpaths[c];
      if (el){
        if (paths[c] && SUB[c]) return null;   /* fill-0 in the flat views */
        return el;
      }
      if (covGhostLit[c] && ghostPaths[c]) return ghostPaths[c];
      return null;
    }
    function coveredBy(z){
      if (covCache[z] !== undefined) return covCache[z];
      var cands = [];
      var C = (HP[z] || {}).country;
      var e0 = covPaintsEl(C, z);
      if (e0) cands.push(e0);
      var m = MULTI[z];
      if (m && m.countries){
        m.countries.forEach(function(c){
          if (c === C) return;
          var e = covPaintsEl(c, z);
          if (e) cands.push(e);
        });
      }
      var hit = false;
        ...
        if (!pts) hit = !paths[z];
          ...
          try {
            pts.forEach(function(pt){
              for (var i = 0; i < cands.length; i++){
                if (cands[i].isPointInFill(pt)){ inC++; return; }
              }
            });
          } catch (e2) { inC = pts.length; }
          hit = inC * 2 > pts.length;
        }
      }
      covCache[z] = hit;
      return hit;
    }
    /* v20 (#6): the shelf pills gate on the HIERARCHY's drawn depth, not
       on this genus's data — the ground exists before the records do
       (grower: "no subdivision pill is available"). Ghost names count. */
    var shelfSet = zones.concat(doubt, Object.keys(ghosts));
    var hasSubzones = shelfSet.some(function(z){ return depthOf(z) >= 2; });
    var hasDivisions = shelfSet.some(function(z){ return depthOf(z) >= 3; });
    function setGhost(zone){
      ...
      em.textContent = zone;
      hover.appendChild(em);
      /* v37: THE CONTEXT TERM IS POLITICAL, NOT GEOGRAPHIC. This
         printed the RAW hierarchy parent, so Himachal Pradesh answered
         "· West Himalaya" - a WGSRPD geographic unit standing where a
         reader expects the country (grower). The v17 law already
         covers it: a geo:1 place is never a row in a political view
         and its children promote to the nearest NON-geo ancestor. A
         real political intermediate is untouched - Sabah keeps
         Malaysian Borneo, Kutai Timur keeps East Kalimantan - because
         neither of those is geo. */
      var ctx = ghosts[zone], gd = 0;
      while (ctx && (HP[ctx] || {}).geo && gd++ < 6) ctx = (HP[ctx] || {}).parent;
      if (!ctx) ctx = (HP[zone] || {}).country;
      hover.appendChild(document.createTextNode(
        (ctx && ctx !== zone ? " · " + ctx : "") +
        " · no species recorded here"));
    }
    /* v10: A PARENT LIGHTS BY ITS RESIDUAL. In Subzones every zone
       with ANY subunit data dimmed to the underlay, so DR Congo - two
       species refined to Katanga, ten recorded only at country level -
       went dark beside Cambodia, which lit only because nothing there
       had been refined at all (grower screenshot). The residual is the
       species recorded in the zone that no subunit on its chain
       carries; a parent drops to the underlay only when that is zero. */
    function chainOf(z){
      /* v17: hier ancestors up to (excluding) the country — geo units
         INCLUDED, because they are the residual carriers ("recorded in
         China Southeast, not placed finer"). */
      var out = [], v = HP[z] || {}, cur = v.parent;
      for (var i = 0; i < 6 && cur && cur !== v.country; i++){
        out.push(cur);
        cur = (HP[cur] || {}).parent;
      }
      return out;
    ...
    function residuals(){
      ...
      RESID = {};
      var inv = zoneSpecies();
      var refined = {};             /* parent -> {epithet: 1} */
      Object.keys(inv).forEach(function(z){
        if (admDepth(z) < 1) return;        /* v17: political, not SUB */
        chainOf(z).forEach(function(p){
          var r = refined[p] = refined[p] || {};
          inv[z].forEach(function(ep){ r[ep] = 1; });
        });
      });
      Object.keys(PARENTS).forEach(function(p){
        var r = refined[p] || {}, n = 0;
        (inv[p] || []).forEach(function(ep){ if (!r[ep]) n++; });
        RESID[p] = n;
      });
      return RESID;
    }
    ...
    function groupsFor(view){
      ...
      if (view === "continents" || view === "regions"){
        /* v17: grouped from the HIERARCHY, not from genus-geo's WGSRPD
           level-1 rollup. The genus map was the last surface saying
           "Tropical Asia / Temperate Asia / Australasia" while the
           journal map, the tags and the hierarchy's own vocabulary note
           all speak the site's six continents (grower: "tropical asia
           and temperate asia are not really two things"). Regions get
           the editorial merges — a region of ONE country is a label,
           not a grouping. Counts are DISTINCT species per group. */
        var RMERGE = { "China": "East Asia", "Eastern Asia": "East Asia",
                       "Mongolia": "East Asia", "Australia": "Australasia",
                       "New Zealand": "Australasia" };
        var nameOf = function(z){
          var h = HP[z] || {};
          if (view === "continents") return h.continent || null;
          var r = h.region || null;
          return r ? (RMERGE[r] || r) : null;
        };
        var inv0 = zoneSpecies(), grpSeen = {};
        zones.forEach(function(z){
          var nm = nameOf(z);
          if (!nm) return;
          var seen = grpSeen[nm] = grpSeen[nm] || {};
          (inv0[z] || []).forEach(function(ep){ seen[ep] = 1; });
        });
        zones.forEach(function(z){
          var nm = nameOf(z);
          if (nm) m[z] = { name: nm,
                           count: Object.keys(grpSeen[nm] || {}).length || 1,
                           merged: 1 };
        });
        /* a fallback for a place the hierarchy does not know: the old
           per-genus grouping still names it */
        var src = (view === "continents" ? g.continents : g.regions) || {};
        Object.keys(src).forEach(function(nm){
          (src[nm].members || []).forEach(function(z){
            if (paths[z] && !m[z]) m[z] = { name: nm, count: src[nm].count, merged: 1 };
          });
        });
      }
      if (view === "countries"){
        /* v9: COUNTRIES FINALLY GROUPS BY COUNTRY. This view was the
           zones grouping with borders switched on, so "Peninsular
           Malaysia", "Borneo" and "Sumatra" each posed as a country
           (grower: "the countries selector is a mess"). Every zone now
           files under its hierarchy country; a multi-country unit with
           no single country - Borneo, New Guinea - honestly stands as
           itself. Counts are DISTINCT SPECIES per country (a species
           on Sumatra and Java is one Indonesian species, not two). */
        var HC = (hier && hier.places) || null;
        if (HC){
          var byC = {}, home = {};
          zones.forEach(function(z){
            /* v17: EVERYTHING groups by its own country — a subunit no
               longer "rides its zone", because the ride is what filed
               BRUNEI under Borneo's biggest neighbour. Brunei is a
               country and now stands as one (grower ruling). Members
               only add species; the fill still comes from the zone
               paths, so nothing double-draws. */
            var c = (HC[z] && HC[z].country) || (MULTI[z] && MULTI[z].main) || z;
            home[z] = c;
            (byC[c] = byC[c] || []).push(z);
            /* v13: a multi-country zone counts toward every country on it */
            if (MULTI[z]) MULTI[z].countries.forEach(function(cc){
              if (cc !== c) (byC[cc] = byC[cc] || []).push(z);
            });
          });
          countryZones = byC;
          var inv = zoneSpecies();
          Object.keys(byC).forEach(function(c){
            var seen = {};
            byC[c].forEach(function(z){
              (inv[z] || []).forEach(function(ep){ seen[ep] = 1; });
            });
            var n = Object.keys(seen).length || 1;
            var merged = byC[c].length > 1 ? 1 : 0;
            byC[c].forEach(function(z){
              if (home[z] === c) m[z] = { name: c, count: n, merged: merged };
            });
            Object.keys(SPLIT).forEach(function(k){
              if (SPLIT[k].country === c) m[k] = { name: c, count: n, merged: merged, split: 1 };
            });
          });
      ...
      zones.forEach(function(z){ if (!m[z]) m[z] = { name: dispName(z), count: g.places[z] }; });
      return m;
    }

    function ramp(count, max){
      if (max <= 1) return 0.78;
      /* v36: THE FLOOR IS THE VISIBLE FLOOR. At 0.16 the lowest step
         composited to rgb(72,82,63) over the rgb(19,26,21) ground -
         2.15:1, and 1.76:1 on a genus with a high maximum - so a
         country WITH a record read as unlit (grower: "india doesn't
         light up"; it took a pixel sample to convince me it was
         painted at all). 0.45 puts the lowest step at 3.4-3.8:1, past
         the 3:1 a graphical object needs. The top stays 0.84 so the
         hover highlight keeps its distance from the hottest data. */
      return 0.45 + 0.39 * (Math.log(1 + count) / Math.log(1 + max));
    }
    /* v26: the ramp value pre-composited over the ground (#131a15) -
       painted at opacity 1 it renders the same pixels as the old
       translucent fill on flat ground, but overlaps cannot double. */
    function compFill(fo){
      return "rgb(" + Math.round(19 + (175 - 19) * fo) + "," +
                      Math.round(26 + (192 - 26) * fo) + "," +
                      Math.round(21 + (144 - 21) * fo) + ")";
    }

    var groupMap = {};
    var climOn = 0;             /* v40: the climate MODE, not a view */
    ...
    function openPanel(zone, doubtful){
      ...
      if (doubtful){
        ...
        /* v9: an "open" with nothing to show IS a dismissal - most of
           the frame is dark neighbouring land that carries a data-zone,
           so this is what a click-away actually hits */
        if (!gr){ closePanel(); return; }
        title = gr.name;
        ...
        /* v14: in Subzones a first-shelf unit drawn whole lists its
           divisions' species too */
        if ((currentView === "subzones" && depthOf(zone) === 1) ||
            (currentView === "subzones2" && depthOf(zone) === 2)){
          Object.keys(inv).forEach(function(z){
            if (chainOf(z).indexOf(zone) >= 0) inv[z].forEach(function(ep){ seen[ep] = 1; });
          });
        }
        /* v13: in Countries a shared zone (Borneo) belongs to every
           country on it, not only the one its polygon joined */
        if (currentView === "countries" && countryZones && countryZones[gr.name]){
          countryZones[gr.name].forEach(function(z){
            (inv[z] || []).forEach(function(ep){ seen[ep] = 1; });
          });
        }
      ...
      eps.sort();
      if (!eps.length){ closePanel(); return; }
      closePanel();
      ...
      journalSlugs().then(function(set){
        ...
        eps.forEach(function(ep){
          ...
          if (set[slug]){
            ...
            a.href = slug;
            a.target = "_blank";            /* v10: the map stays put */
            a.rel = "noopener";
            a.appendChild(it);
            li.appendChild(a);
            linked++;
    ...
    function setHot(zone){
      ...
      svg.classList.toggle("apgm--hov", !!gr);      /* v20 (#7) */
      order.forEach(function(z){
        paths[z].classList.toggle("is-hot",
          !!(gr && groupMap[z] && groupMap[z].name === gr.name));
      });
      Object.keys(spaths).forEach(function(k){
        spaths[k].classList.toggle("is-hot",
          !!(gr && groupMap[k] && groupMap[k].name === gr.name));
      });
      Object.keys(ghostPaths).forEach(function(k){     /* v18.1 */
        ghostPaths[k].classList.toggle("is-hot",
          !!(gr && groupMap[k] && groupMap[k].name === gr.name));
      });
      ...
      /* v16: on a container the number is what STOPS there */
      if (currentView === "subzones" || currentView === "subzones2" ||
          currentView === "divisions"){
        var _ctr = !!PARENTS[zone] && (currentView === "divisions" ||
                   depthOf(zone) < (currentView === "subzones2" ? 2 : 1));
        if (_ctr && residuals()[zone]){
          tail = " · " + residuals()[zone] + " not tagged to a " +
                 (currentView === "divisions" ? "division" :
                  currentView === "subzones2" ? "subzone" : "zone");
        } else if (_ctr && !residuals()[zone] && families()[zone]){
          /* v18.1: a container with NOTHING stopping at it - every
             record refined below - says what it holds, not "0" */
          tail = " · " + families()[zone] + " species recorded below";
        }
      }
      ...
      if (climOn){ climTail(); climHotEdge(); }        /* v40 / v41 */
    }
    function setDoubt(zone){
      ...
      hover.innerHTML = "";
      var em = document.createElement("em");
      em.textContent = dispName(zone);   /* v17: Sarawak – Malaysian Borneo */
      hover.appendChild(em);
      hover.appendChild(document.createTextNode(" · " +
    ...
    function apply(view){
      ...
      var divView = view === "divisions";
      var sub2 = view === "subzones2";
      var subView = view === "subzones" || sub2 || divView;
      /* v18: the ROW DEPTH of this shelf - 1 for Zones, 2 for Subzones,
         everything for Divisions. A unit AT it is a row; above it with
         drawn children, a container band; a shallower childless leaf
         keeps the ramp (Johor has no divisions and must not go dark in
         Subzones); deeper hides. */
      var rowD = divView ? 99 : (sub2 ? 2 : 1);
      svg.classList.toggle("apgm--range", view === "range");   /* v23 */
      svg.classList.toggle("apgm--solid", !subView);            /* v26 */
      svg.classList.toggle("apgm--grp",
        view === "continents" || view === "regions");            /* v25 */
      svg.classList.toggle("apgm--vcty", view === "countries");  /* v30 */
      /* v40: climate is a MODE, not a view - it rides whatever pill is
         selected, so the geo rows stay hoverable underneath it. */
      svg.classList.toggle("apgm--vclim", !!climOn);
      climHost.style.display = climOn ? "" : "none";
      if (climOn){
        climLoad().then(function(im){ climBuild(im); climLegend(); climHotEdge(); });
      } else if (climClipView !== null){
        climClipView = null;              /* re-cut on the way back in */
      }
      svg.classList.toggle("apgm--vrgn", view === "regions");     /* v31 */
      svg.classList.toggle("apgm--vcnt", view === "continents");  /* v42 */
      svg.classList.toggle("apgm--vsub", subView);
      svg.classList.toggle("apgm--vsub2", sub2);
      svg.classList.toggle("apgm--vdiv", divView);
      groupMap = groupsFor(subView ? "zones" : view);
      /* v31: only a PAINTED region wears its border - a genus absent
         from a region must not ring empty dark ground. */
      if (view === "regions" || view === "continents"){
        if (!gbBuilt[view]){
          gbBuilt[view] = 1;
          /* v32: THE BORDER FOLLOWS THE PAINTED FOOTPRINT. A segment
             draws when a side actually paints: a recorded zone, or a
             zone whose COUNTRY lights (the coarse China record paints
             all of China). Region-level gating ringed unpainted
             Pakistan and Mongolia with their regions' political rims
             (grower). Static per genus, so built once per view. */
          var ZSET = {}, CLIT = {};
          zones.forEach(function(z){
            ZSET[z] = 1;
            var hz = HP[z] || {};
            if (hz.country) CLIT[hz.country] = 1;
            if (MULTI[z] && MULTI[z].countries)
              MULTI[z].countries.forEach(function(c){ CLIT[c] = 1; });
          });
          Object.keys(gbSrc[view]).forEach(function(g){
            var d = "";
            (gbSrc[view][g] || []).forEach(function(seg){
              var on = !seg.z.length || seg.z.some(function(z2){
                return ZSET[z2] || CLIT[(HP[z2] || {}).country];
              });
              if (on) d += seg.d;
            });
            var pp = gbPaths[view + "|" + g];
        ...
        zones.forEach(function(z){
          var gr0 = groupMap[z];
          if (gr0) gbLit[gr0.name] = 1;
        });
        Object.keys(gbPaths).forEach(function(k){
          var pre = view + "|";
          gbPaths[k].classList.toggle("apgm-rb--on",
            k.indexOf(pre) === 0 && !!gbLit[k.slice(pre.length)]);
        });
      } else {
        Object.keys(gbPaths).forEach(function(k){
          gbPaths[k].classList.remove("apgm-rb--on");
        });
      }
      /* v14: in Subzones a first-shelf unit is drawn WHOLE (its
         divisions hidden) and carries its family; in Divisions it is a
         parent like any other and lights by its residual */
      var countOf = function(z){
        var d = depthOf(z);
        if (subView && !divView && d === rowD) return families()[z] || groupMap[z].count;
        if (subView && PARENTS[z]) return residuals()[z];
        return groupMap[z].count;
      };
      if (subView && !divView){
        order.forEach(function(z){
          if (depthOf(z) === rowD && groupMap[z]) groupMap[z] = { name: dispName(z), count: countOf(z) };
        });
      }
      /* ⚠⚠⚠ v16 — A CONTAINER NEVER SETS OR JOINS THE RAMP. v10 let a
         parent light by its residual, and one shelf deeper that put
         Sarawak's 23 unrefined records ON THE SAME SCALE as Kapit's 1:
         the parent took the legend maximum, and its unlit divisions -
         fill-opacity 0, so the parent shows through - read BRIGHTER
         than the single division that actually holds a record (grower,
         journal screenshot; the same maths runs here). Containment is
         a BAND now, never a ramp value: depth keeps the card's ladder,
         a residual brightens it one step, and every step stays below
         the dimmest lit shape.
         ⚠ "Container" is per VIEW: in Subzones a depth-1 unit is drawn
         WHOLE (its divisions hidden), so it is a leaf there and keeps
         the ramp with its family count - only in Divisions does it
         become a container. */
      var isCtr = function(z){
        return (!!PARENTS[z] || !!HASKID[z]) && (divView || depthOf(z) < rowD);
      };
      var max = 1;
      Object.keys(groupMap).forEach(function(z){
        if (subView && !divView && depthOf(z) > rowD) return;
        if (subView && isCtr(z)) return;
        var c = countOf(z);
        if (c > max) max = c;
      });
      /* v18.1: family rungs join the scale before any fill is painted */
      if (subView && !divView){
        Object.keys(ghostPaths).forEach(function(k){
          if (depthOf(k) === rowD){
            var fam = families()[k] || 0;
            if (fam > max) max = fam;
          }
        });
      }
      /* ===== v22 - THE POLITICAL VIEWS COMPLETE THEIR COUNTRIES =====
         Australia showed only Queensland and New South Wales because
         nothing in the archive carries a country-grade Australia tag -
         China fills whole only because a coarse 'China' record exists.
         In Continents/Regions/Countries, a country whose zones hold
         data now lights its GHOST shape at its group's fill, and the
         member zones dissolve into it through the covered test - the
         same rule as everywhere else, the ghost simply counts as
         painted ground. Range is untouched on purpose: it is the
         RECORDED range, and painting Western Australia there would
         claim ground the genus does not reach. */
      var ghostLit = {};
      covCache = {};                       /* ghost-litness is per view */
      covOK = 0; covFail = 0;                              /* v35 */
      covGhostLit = ghostLit;
      /* v38: the climate view is DATA-TRUE like Range - no country
         ghosts. Lighting Australia whole from Queensland is right when
         the unit is the country; here the unit is the ground itself,
         and painting climate over land the genus never reaches would
         be a straight falsehood. */
      if (!subView && view !== "range" && hier && hier.places){
        var invGL = zoneSpecies();
        var byCG = {};
        var glAdd = function(c, z){
          if (!c || c === z) return;
          if (paths[c] || dpaths[c]) return;   /* the country draws itself */
          if (!ghostPaths[c]) return;
          (byCG[c] = byCG[c] || []).push(z);
        };
        zones.forEach(function(z){
          var hc = hier.places[z];
          var c = (hc && hc.country) || (MULTI[z] && MULTI[z].main) || z;
          glAdd(c, z);
          /* v13's law: a multi-country zone counts toward every country */
          if (MULTI[z] && MULTI[z].countries){
            MULTI[z].countries.forEach(function(cc){ if (cc !== c) glAdd(cc, z); });
          }
        });
        Object.keys(byCG).forEach(function(c){
          var seen = {};
          byCG[c].forEach(function(z){
            (invGL[z] || []).forEach(function(ep){ seen[ep] = 1; });
          });
          var n = Object.keys(seen).length;
          if (!n) return;
          var mem = byCG[c][0], mg = groupMap[mem];
          var fill, entry;
          if (view === "countries"){
            fill = ramp(n, max);
            entry = { name: dispName(c), count: n, __ghost: 1 };
          } else {
            /* v25: the ghost JOINS the view's group. Naming it after
               its country made "China · 7" read as its own continent
               AND its own region, kept the Asia hover from lighting
               it (setHot matches group NAMES), and ramped Australia
               at its own 2 against Oceania's 16. */
            fill = (mg && mg.count) ? ramp(mg.count, max) : ramp(n, max);
            entry = mg ? { name: mg.name, count: mg.count, __ghost: 1 }
                       : { name: dispName(c), count: n, __ghost: 1 };
          }
          ghostLit[c] = fill;
          groupMap[c] = entry;
        });
      ...
      order.forEach(function(z){
        var gr = groupMap[z];
        var fo, d = depthOf(z);
        /* v23: no coverage arithmetic in Range - solid fills cannot
           double, so every recorded shape simply paints. */
        var cov = !subView && view !== "range" && coveredBy(z);
        paths[z].classList.toggle("apgm-multi", !!MULTI[z]);
        /* v28: in Divisions a container's ground belongs to the shelf -
           hovering a regency answered "East Kalimantan" wherever the
           regency ghosts leave a gap. The band still paints (the
           residual has to live somewhere) but takes no pointer. */
        paths[z].classList.toggle("apgm-ctr", !!(subView && isCtr(z)));
        paths[z].classList.toggle("apgm-cov", cov);
        if (climOn){
          /* v38: the fills recede to a near-ground green so the bands
             carry the colour, and ground the raster does not classify
             still reads as land rather than as open sea. */
          fo = 1;
          paths[z].style.setProperty("--zf", "rgb(40,48,40)");
        } else if (view === "range"){
          /* v25: EVERYTHING paints - the SUB suppression was an
             anti-stacking measure, obsolete at opacity 1, and it hid
             the islets a subunit's own shape carries that its
             parent's geometry misses (grower: "little small islands
             appear" on hover). */
          fo = 1;
          /* the pre-composited .8 wash over the ground, now opaque */
          paths[z].style.setProperty("--zf", "rgb(144,159,119)");
        } else if (subView) {
          paths[z].style.removeProperty("--zf");
        }
        if (view === "range" || climOn)   ;
        else if (subView && !divView && d > rowD) fo = 0;
        else if (subView && isCtr(z)) fo = d >= 1
                                       ? ((residuals()[z] || families()[z]) ? 0.28 : 0.18)
                                       : ((residuals()[z] || families()[z]) ? 0.16 : 0.07);
        else if (subView && !divView && d === rowD) fo = ramp(countOf(z), max);
        else if (subView)       fo = ramp(gr.count, max);
        else if (SUB[z] || cov) fo = MULTI[z] ? 0 : ramp(gr.count, max);
        else                    fo = ramp(gr.count, max);
        /* v26: SOLID GROUPED FILLS - the "country shapes visible
           inside a big geography" were overlap blends (grower guessed
           it exactly): translucent members double where they overlap.
           Solid pre-composited fills render identically and cannot. */
        if (!subView && view !== "range"){
          paths[z].style.fillOpacity = fo > 0 ? 1 : 0;
          if (fo > 0) paths[z].style.setProperty("--zf", compFill(fo));
        } else {
          paths[z].style.fillOpacity = fo;
        }
        /* v30: ONE BORDER, ONE TREATMENT. A single-shape country
           (Laos) never earned the merged class, so it kept the base
           cream stroke at .45 - brighter than the uniform border and
           varying with the fill beneath ("Laos seems almost
           highlighted", round two). Every painted country now wears
           the same class, colour, width and opacity. */
        var uniB = view === "countries" && fo > 0 && !SUB[z] && !cov;
        paths[z].classList.toggle("apgm-zone--merged", !!gr.merged || uniB);
        /* v17: a merged member's stroke is the COMPOSITED fill colour —
           green at this opacity over the #131a15 ground — so the seam
           between two same-group shapes paints itself invisible instead
           of hiding behind stroke-opacity:0 and leaking on hover. */
        /* v29: every painted merged shape carries the seam stroke -
           the !cov exclusion predates covered shapes painting, and it
           left their antialias cracks uncovered ("why are we drawing
           borders for no reason in continents" = those cracks). And
           the Countries border is now ONE colour: leaning each
           country's own fill made bright Laos wear a brighter outline
           than its neighbours ("Laos seems almost highlighted"). */
        if (gr && (gr.merged || uniB) && fo > 0){
          if (view === "range"){
            paths[z].style.setProperty("--mst", "rgb(144,159,119)");
          } else if (view === "countries"){
            /* v42: the same cream as Zones and the grouped borders */
            paths[z].style.setProperty("--mst", "rgba(239,240,232,.5)");
          } else {
            paths[z].style.setProperty("--mst", compFill(fo));
          }
        } else {
          paths[z].style.removeProperty("--mst");
        }
      });
      Object.keys(spaths).forEach(function(k){
        var sg = groupMap[k];
        /* v20 (#1): a split exists to give a multi-country island's
           ground to its other countries — but when that country's OWN
           shape is drawn it already paints this ground, and the split
           on top just doubles the fill along the state line. */
        var dup = SPLIT[k] && (paths[SPLIT[k].country] || dpaths[SPLIT[k].country]);
        spaths[k].style.fillOpacity = (view === "countries" && sg && !dup) ? ramp(sg.count, max) : 0;
        spaths[k].classList.toggle("apgm-zone--merged", !!(sg && sg.merged));
      });
      /* v23: the doubt hatch over painted ground. In Range the native
         fill beneath is solid, so the hatch fill only ever doubled; in
         the political views the lit country ghosts are the ground. The
         dashes - the actual message - stay. */
      Object.keys(dpaths).forEach(function(dz){
        var dcov = view === "range" ? !!paths[dz]
                 : (!subView && coveredBy(dz));
        dpaths[dz].classList.toggle("apgm-cov", !!dcov);
      });
      /* v18.1: A DATA-LESS RUNG WITH DATA BELOW IT (Malaysian Borneo).
         At its own shelf it lights by its FAMILY - the rows' rule; above
         its shelf, with drawn data beneath, it bands - the container
         rule. Every other ghost stays an invisible name-carrier. */
      Object.keys(ghostPaths).forEach(function(k){
        var el = ghostPaths[k];
        el.classList.remove("apgm-famrow");
        el.classList.remove("apgm-zone--merged");
        el.style.removeProperty("--fam");
        el.style.removeProperty("--mst");
        el.style.removeProperty("--zf");
        if (groupMap[k] && groupMap[k].__ghost && !ghostLit[k]) delete groupMap[k];
        if (!subView){
          if (ghostLit[k]){
            el.classList.add("apgm-famrow");
            /* v26: solid like the members - a translucent ghost under
               a differently-grouped zone blended into a bright strip
               (west New Guinea). */
            el.style.setProperty("--fam", 1);
            /* v41: in climate mode a lit ghost is GROUND for the
               bands, not a heat value - the raster carries the colour */
            el.style.setProperty("--zf", climOn ? "rgb(40,48,40)"
                                                : compFill(ghostLit[k]));
            /* v26: the crack-filler - two solid ghost fills abutting
               (Malaysia against Indonesia across Borneo) leave an
               antialias hairline unless a same-colour stroke paints
               the seam, exactly like every merged member. */
            el.classList.add("apgm-zone--merged");
            /* v30: in Countries the ghost wears the UNIFORM border,
               not its own fill - a fill-coloured stroke is no border,
               so ghost countries stood borderless beside bordered
               data countries. */
            el.style.setProperty("--mst", view === "countries"
              ? "rgba(239,240,232,.5)" : compFill(ghostLit[k]));
          }
          return;
        }
        var d = depthOf(k), fam = families()[k] || 0;
        if (!fam) return;
        if (!divView && d === rowD){
          el.classList.add("apgm-famrow");
          el.style.setProperty("--fam", ramp(fam, max));
          groupMap[k] = { name: dispName(k), count: fam, __ghost: 1 };
        } else if (PARENTS[k] && (divView || d < rowD)){
          el.classList.add("apgm-famrow");
          el.style.setProperty("--fam", d >= 1 ? (residuals()[k] ? 0.28 : 0.18)
                                               : (residuals()[k] ? 0.16 : 0.07));
          groupMap[k] = { name: dispName(k), count: residuals()[k] || 0, __ghost: 1 };
        }
      });
      ...
      if (climOn){
        ...
        legend.appendChild(el("span", "apgm__legend-t", "species per " +
          (view === "continents" ? "continent" :
           ...
           divView ? "division" : sub2 ? "subzone" : "zone")));
      }
      ...
      setHot(null);
      /* v35: THE FIRST PAINT NEVER KNEW WHAT WAS COVERED. coveredBy()
         is geometric, and this function first runs before layout, when
         isPointInFill answers nothing - so every unit came back "not
         covered" and the opening frame followed a different rule from
         every frame after it (live: cov=0 at load on every genus, then
         96 covered zones on Alocasia the moment a view was switched;
         a MULTI unit like Borneo painted itself over the country
         ghosts that should have replaced it). If NO probe found
         geometry while some wanted it, the map is simply not laid out
         yet - drop the cache and paint again next frame. Bounded, and
         it cannot spin on a single degenerate shape because a pass
         with any successful probe never retries. */
      if (covFail && !covOK && covRetry < 20){
        covRetry++;
        requestAnimationFrame(function(){ covPt = {}; apply(view); });
      }
    }

    /* v38: the climate key replaces the gradient - these are
       CATEGORIES, not a scale, so a bar would be a lie. Only the zones
       the genus's own range touches are listed. */
    function climLegend(){
      if (!climOn) return;
      legend.innerHTML = "";
      if (climFailed){
        legend.appendChild(el("span", "apgm__legend-t", "climate layer unavailable"));
        return;
      }
      if (!climMeta || !climPresent){
        legend.appendChild(el("span", "apgm__legend-t", "reading the climate bands\u2026"));
        return;
      }
      climPresent.forEach(function(zi){
        var z = climMeta.zones[zi];
        if (!z) return;
        var chip = el("span", "apgm__clim-key");
        var sw = el("span", "apgm__clim-sw");
        sw.style.background = z.hex;
        chip.appendChild(sw);
        chip.appendChild(document.createTextNode(z.name));
        if (z.t) chip.setAttribute("title", z.name + " \u00b7 " + z.t[0] + " to " +
          z.t[1] + " \u00b0C" + (z.rh ? " \u00b7 " + z.rh[0] + "\u2013" + z.rh[1] +
          "% humidity" : ""));
        legend.appendChild(chip);
      });
    }

    /* v17: the pill row is the POLITICAL ladder. "Zones" (raw WGSRPD
       level 3) is retired — its units still exist as tags, cards, and
       the geo containers inside States. "Subzones" is renamed States;
       the internal view id stays "subzones" so every existing branch,
       harness and deep link keeps working. */
    /* v18: THREE SHELVES, in the site's own vocabulary (the wiki's
       Geography-Shelves speaks zones/subzones/divisions). "Zones" =
       first shelf below country (Malaysian Borneo, Kalimantan, Johor,
       a Thai province); "Subzones" = second (Sarawak, Sabah, West
       Kalimantan); "Divisions" = the deepest (Kapit, Labuan). Internal
       ids: subzones / subzones2 / divisions - the first keeps its old
       id so every branch and harness still works. */
    [["range", "Range"], ["continents", "Continents"], ["regions", "Regions"],
     ["countries", "Countries"],
     ["subzones", "Zones"], ["subzones2", "Subzones"],
     ["divisions", "Divisions"]].forEach(function(v){
      /* v7: a genus with no subunit data gets no Subzones pill —
         the view would be Zones twice; v14: no Divisions pill without
         a second shelf to show */
      if (v[0] === "subzones" &&
          !shelfSet.some(function(z){ return depthOf(z) >= 1; })) return;
      if (v[0] === "subzones2" && !hasSubzones) return;
      if (v[0] === "divisions" && !hasDivisions) return;
    ...
    svg.addEventListener("mouseover", function(ev){
      ...
      /* v17: a SPLIT (Brunei) outranks its ghost — both paths exist for
         the same name, and the ghost's "no species recorded here" was
         shadowing the country readout the split carries. */
      if (paths[z] || ((spaths[z] || ghosts[z]) && groupMap[z])) setHot(z);
      else if (ghosts[z]) setGhost(z); else setDoubt(z);
    });
    ...
    svg.addEventListener("click", function(ev){
      ...
      if (!t || !t.getAttribute) return;
      var z = t.getAttribute("data-zone");
      /* v9: EMPTY GROUND DISMISSES. The panel closed only via its own
         x; a click on open sea or a drag left it standing (grower:
         "it won't disappear when you click away or drag the map"). */
      if (!z || ghosts[z]){ closePanel(); setHot(null); return; }
      openPanel(z, !paths[z] && !spaths[z] && !(ghosts[z] && groupMap[z]));
    });

    apply("countries");   /* v17 default: the political anchor view */
  ...
  function build(){
    ...
    feeds().then(function(res){
      ...
      var shapes = res[0], geo = res[1], hier = res[2] || null;
      ...
      try { render(host, genus, shapes, geo, g, hier); }
      catch (err){
...
(function () {
  ...
  var PAGES = [
    /* genus pages - these prefixes also cover their
       "-reproduction" companions, which is why those are not listed.
       The 2026-08-20 rename from "-pollination" needed no change here:
       prefix matching means "/alocasia" covers whatever the companion
       page is called.
       v134 adds the four newest guides. Checked against the published
       search-index before adding: each of the four prefixes catches
       ZERO other indexed URL, exactly like the eight already here. */
    "/alocasia",
    "/amorphophallus",
    "/anthurium",
    "/arisaema",
    ...
    /* reproduction hub (renamed from /aroid-pollination 2026-08-20) */
    "/aroid-reproduction",
...
(function () {
  ...
  /* ⚠⚠ THE KEY MUST BE THE HREF OF A REAL ROW IN THE FOLDER DROPDOWN.
     It is matched with a[href="..."]; if no row has that exact href,
     nothing is wired and the flyout silently does not exist. That is
     what happened between the 2026-08-20 slug rename and FILE v132:
     the key still read "/aroid-pollination" after the nav row had
     become "/aroid-reproduction", so the flyout stopped building and
     its eight links pointed at 404s. If the flyout disappears, check
     THIS KEY against the live nav row first.

     ⚠ pending:true = the guide is written but not yet published.
     Those rows are filtered out below, so an unpublished page never
     ships a dead link into a sitewide menu. WHEN THE PAGE GOES LIVE,
     DELETE THE FLAG - nothing else needs changing.
     Verified by HTTP sweep 2026-08-20: /dracunculus-reproduction 200;
     /dieffenbachia-reproduction, /helicodiceros-reproduction and
     /homalomena-reproduction all 404.

     Order: the cross-genus page first, then the genera alphabetically. */
  /* v144: TWO LEVELS. A row is either a LINK ({label, href}) or a
     GROUP ({label, children:[...]}). A group has no href, does not
     navigate, and opens its own flyout beside the first one.

     WHY: the flat list reached fourteen rows and was taller than the
     dropdown it hung off. The five genera people actually search for
     stay on the first level; everything else moves one step in.
     The split is EDITORIAL, set 8.21.26, and is not derived from
     anything in the data - do not "fix" it by sorting on paper counts
     or page length. Arum has the largest guide on the site and is
     deliberately in the second level, because almost nobody arrives
     looking for it by name.

     v144 also DROPPED the `pending` flag. Until v143 an unpublished
     guide carried `pending: true` and was filtered out here, which
     hid three finished guides. Ruled 8.21.26: show them anyway. An
     unpublished slug 404s and that is accepted - a visible link to a
     page that is coming beats a menu that silently omits a third of
     the series. To hide a row now, delete it. */
  var SUBNAV_ALL = {
    /* v135: THE MORPHOLOGY FLYOUT. v137: FOUR rows now - Monstera and
       Philodendron were published 8.27.26 and both were curled 200
       before being added here. The list still does NOT follow the
       no-filtering ruling that governs the reproduction list below
       it; see the NAV v135 banner at the top of this file for why.
       ⚠ ADD A GENUS ONLY WHEN ITS GUIDE IS PUBLISHED. Fifteen more
       are written up as "In preparation" cards on the hub and their
       slugs do not exist yet. Curl the href before you add it.
       ⚠ Order is the cross-genus page first, then the genera
       ALPHABETICALLY - not by how good the guide is or how much of
       the literature stood behind it. */
    "/aroid-morphology": [
      { label: "Alocasia Morphology",     href: "/alocasia-morphology" },
      { label: "Anthurium Morphology",    href: "/anthurium-morphology" },
      { label: "Monstera Morphology",     href: "/monstera-morphology" },
      { label: "Philodendron Morphology", href: "/philodendron-morphology" }
    ],

    "/aroid-reproduction": [
      { label: "Why Some Crosses Fail",       href: "/chromosomes-and-crossing" },
      { label: "Alocasia Reproduction",       href: "/alocasia-reproduction" },
      { label: "Amorphophallus Reproduction", href: "/amorphophallus-reproduction" },
      { label: "Anthurium Reproduction",      href: "/anthurium-reproduction" },
      { label: "Monstera Reproduction",       href: "/monstera-reproduction" },
      { label: "Philodendron Reproduction",   href: "/philodendron-reproduction" },
      { label: "Other Genera Reproduction",   children: [
        { label: "Arisaema Reproduction",        href: "/arisaema-reproduction" },
        { label: "Arum Reproduction",            href: "/arum-reproduction" },
        { label: "Dieffenbachia Reproduction",   href: "/dieffenbachia-reproduction" },
        { label: "Dracunculus Reproduction",     href: "/dracunculus-reproduction" },
        { label: "Helicodiceros Reproduction",   href: "/helicodiceros-reproduction" },
        { label: "Homalomena Reproduction",      href: "/homalomena-reproduction" },
        { label: "Schismatoglottis Reproduction", href: "/schismatoglottis-reproduction" },
        { label: "Spathiphyllum Reproduction",   href: "/spathiphyllum-reproduction" }
      ] }
    ]
  };

  /* No filtering any more. Kept as a named alias so both consumers
     still read one symbol and a future filter has an obvious home. */
  var SUBNAV = SUBNAV_ALL;
  ...
  function wire(item, panel, link, entries) {
    ...
    /* Surfaces that must all keep the FIRST-level flyout alive. A
       second-level panel is a child of the folder item, not of the
       flyout, so moving the cursor onto it counts as leaving the
       flyout - without adding it here the first level would shut
       the moment the reader stepped into the second. */
    var keepAlive = [row, fly];
    var subs = [];
    ...
    entries.forEach(function (entry) {
      ...
      /* A GROUP row. No href: it is a parent, not a destination, and
         giving it one would send the reader somewhere that does not
         exist. tabindex keeps it in the tab order all the same. */
      a.className = "ap-sub-group";
      a.setAttribute("tabindex", "0");
      a.setAttribute("role", "button");
      a.setAttribute("aria-haspopup", "true");
      a.setAttribute("aria-expanded", "false");
      var c2 = document.createElement("span");
      c2.className = "ap-subcue";
      c2.setAttribute("aria-hidden", "true");
      a.appendChild(c2);
      fly.appendChild(a);

      var fly2 = document.createElement("div");
      fly2.className = "ap-subnav ap-subnav--l2";
      ...
      /* Same escape hatch as level one: the folder ITEM, so it clears
         the dropdown's overflow clip and its backdrop root. */
      item.appendChild(fly2);
      keepAlive.push(fly2);
      subs.push({ link: a, fly: fly2 });
    });
    ...
    function shut() {
      ...
      /* v135: REFERENCE-COUNTED, NOT UNCONDITIONAL. Two rows in this
         folder now own a flyout, and both used to add and remove this
         one class blindly. Crossing from the Morphology row to the
         Reproduction row leaves Morphology's 260ms close timer armed;
         when it fired it took the hold away while Reproduction's
         flyout was open. Ask the DOM instead: the class goes only when
         nothing under this folder is still showing. `fly` has already
         lost ap-sub-open above, so this reads the state AFTER the
         close, and a second-level panel counts too - if one is up, the
         dropdown behind it must stay up. */
      if (!item.querySelector(".ap-subnav.ap-sub-open")) {
    ...
    /* ---- second level ------------------------------------------
       place() is reused unchanged. It only reads left/right/width
       off the surface it is anchoring beside, so handing it the
       FIRST flyout in place of the dropdown panel positions the
       second one against it correctly, including the
       open-to-the-right fallback near the viewport edge. */
    subs.forEach(function (s2) {
      var t2 = null;
      function open2() {
        clearTimeout(t2);
        open();                    /* the first level stays lit under it */
        if (place(item, fly, s2.link, s2.fly)) {
          s2.fly.classList.add("ap-sub-open");
          s2.link.classList.add("ap-sub-parent-row");
          s2.link.setAttribute("aria-expanded", "true");
        }
      }
      function close2() { clearTimeout(t2); t2 = setTimeout(shut2, CLOSE_MS); }
      function shut2() {
        clearTimeout(t2);
        s2.fly.classList.remove("ap-sub-open");
        s2.link.classList.remove("ap-sub-parent-row");
        s2.link.setAttribute("aria-expanded", "false");
      }
      [s2.link, s2.fly].forEach(function (el) {
        el.addEventListener("mouseenter", open2);
        el.addEventListener("mouseleave", close2);
        el.addEventListener("focusin", open2);
        el.addEventListener("focusout", close2);
      });
      /* Stepping onto any OTHER row of the first flyout closes it -
         two open second-level panels at once would overlap. */
      fly.querySelectorAll("a").forEach(function (a) {
        if (a !== s2.link) a.addEventListener("mouseenter", close2);
      });
      s2.link.addEventListener("keydown", function (e) {
        if (e.key === "ArrowLeft" || e.key === "ArrowRight" ||
            e.key === "Enter" || e.key === " ") {
          var f = s2.fly.querySelector("a");
          if (f) { e.preventDefault(); open2(); f.focus(); }
        }
      });
      s2.fly.addEventListener("keydown", function (e) {
        if (e.key === "Escape") { e.preventDefault(); s2.link.focus(); shut2(); }
      });
      /* The whole folder closing takes the second level with it. */
      item.addEventListener("mouseleave", close2);
    });
  ...
  function bindMobile() {
    ...
    Object.keys(SUBNAV).forEach(function (path) {
      ...
      /* Flatten for the overlay: a group becomes one toggle row
         followed by its children, which start hidden. There is no
         hover on a phone, so a second panel is not an option -
         the children fold in and out under their heading instead. */
      var flat = [];
      SUBNAV[path].forEach(function (e) {
        if (e.children) {
          flat.push({ label: e.label, group: true });
          e.children.forEach(function (c) {
            flat.push({ label: c.label, href: c.href, child: true });
          });
        } else {
          flat.push(e);
        }
      ...
      flat.forEach(function (entry) {
        ...
        if (entry.href) {
          ...
          a.removeAttribute("href");        /* a heading, not a link */
          a.setAttribute("role", "button");
          a.setAttribute("tabindex", "0");
          a.setAttribute("aria-expanded", "false");
        }
        if (entry.group)  { c.classList.add("ap-subnav-mobile--group"); }
        if (entry.child)  { c.classList.add("ap-subnav-mobile--l2", "ap-sub2-hidden"); }
        ...
        if (entry.group) { groupRow = c; groupKids = []; }
        else if (entry.child && groupRow) { groupKids.push(c); }
      });

      /* Wire the toggle once, over the children collected above. */
      if (groupRow && groupKids.length) {
        (function (gr, kids) {
          var ga = gr.querySelector("a");
          var openState = false;
          function toggle(e) {
            if (e) { e.preventDefault(); e.stopPropagation(); }
            openState = !openState;
            kids.forEach(function (k) {
              k.classList.toggle("ap-sub2-hidden", !openState);
            });
            gr.classList.toggle("ap-sub2-open", openState);
            if (ga) ga.setAttribute("aria-expanded", openState ? "true" : "false");
          }
          if (ga) {
            ga.addEventListener("click", toggle);
            ga.addEventListener("keydown", function (e) {
...
(function(){
  ...
  /* ================================================================
     ...
     ================================================================ */
  var CFG = {
    /* ── THE POINT-LOCALITY MARKER, IN CSS PIXELS ──────────────────
       A handful of feed places have no polygon (Alabat on Alocasia
       zebrina) and draw as a dot with a leader and a permanent label.
       These are PIXELS ON SCREEN, held constant through zoom and
       through the scroll-collapse - see the note in sizeDots(). */
    MARK_R_PX    : 3.5,    /* dot radius */
    MARK_LEAD_PX : 38,     /* leader length, dot -> label */
    MARK_FS_PX   : 14,     /* label type size */
    /* country labels - see the note in the CSS. Pixels, like the marker. */
    CLAB_FS_PX   : 8.5,    /* country name type size */
    CLAB_GAP_PX  : 5,      /* clearance between a label and its feature */
    CLAB_MAX     : 10,     /* beyond this many lit countries the map is too
                              busy to letter and labels are skipped entirely */
    ...
    /* v149 (8.30.26, RE-APPLIED 9.1.26): the photos themselves, on R2
       behind the site's own domain. Manifest paths resolve against THIS,
       not MANIFEST_BASE - see loadManifest below.
       /!\ THIS LINE IS LOAD-BEARING AND HAS BEEN LOST ONCE. It shipped in
       FILE v198; another lane then built its own v198 from an older master
       and v199-v207 all inherited the gap, so every journal photo 404'd -
       the manifests name content-hashed keys that exist ONLY on R2.
       If you are merging masters, keep this and __base = photos below. */
  ...
  var SCHEMA = [
    ...
    /* v81: THE PARENT PANELS - hybrids only. A named cross shows a
       photograph of each parent beside the parentage, where those photos
       exist. They are pictures of a DIFFERENT TAXON, so they must not join
       the archive: the More photos loop below is an explicit allowlist and
       these roles are deliberately absent from it. Placed here, next to
       Parentage, for the same reason adddist sits with the geography
       cluster - schema order is render order. */
    {key:"ovuleparent",  label:"Ovule parent",  alt:["OVULE PARENT","SEED PARENT","OVULE"]},
    {key:"pollenparent", label:"Pollen parent", alt:["POLLEN PARENT","POLLEN"]},
  ...
  function shapesHd(){
    if (!shapesHdPromise){
      /* F6 (card v148): the shared topology loader — one fetch across
         the card and the genus map, decoded to the same contract this
         function always returned. Falls back to the old direct
         shapes-hd fetch if the loader is somehow absent. */
      shapesHdPromise = (window.__apShapes ? window.__apShapes()
        : fetch(CFG.SHAPES_HD_URL, {mode:"cors"})
            .then(function(r){ if(!r.ok) throw new Error(r.status); return r.json(); }))
        .catch(function(){ return null; });
    }
  ...
  function mergeManifest(parsed, man, title){
    ...
    if (parsed.sections.ovuleparent)  into("ovuleparent",  R["ovule-parent"],  4.4, "ovule-parent");
    if (parsed.sections.pollenparent) into("pollenparent", R["pollen-parent"], 4.6, "pollen-parent");
  ...
  function resolveRegions(tags, distText, data){
    ...
    [["Borneo", ["Sarawak","Sabah","Kalimantan","Brunei"]],
     ...
     /* v138 (8.25.26): BANGLADESH'S SHELVES. The gb-layer batch keyed
        the 8 real divisions plus Moulvibazar district (the Alocasia
        hararganjensis type locality), and a post tags the whole chain -
        Bangladesh + Sylhet + Moulvibazar - so without these rows the
        card showed three sibling chips and lit the whole country over
        its own district. Moulvibazar nests via Sylhet transitively. */
     ["Bangladesh", ["Barisal","Chittagong","Dhaka","Khulna","Mymensingh",
     ...
     ["Bolivia", [
       ...
       "La Paz-BO","Oruro","Pando",
       "Potosí","Santa Cruz","Tarija"
     ]],
     ["Peru", [
       "Amazonas-PE","Apurímac","Arequipa",
     ...
     ["Venezuela", [
       "Amazonas-VE","Anzoátegui","Apure",
       "Aragua","Barinas","Bolívar-VE",
     ...
     /* NEOTROPICS TIER 1, 8.16.26 - Colombia, Ecuador, Panama and
        ...
        with tiers 2-3: two Bolívars, three Amazonas, two Sucres, one
        Colón, and "Panamá Province" against the COUNTRY Panama.
        ⚠ 8.20.26: these qualifiers were rewritten from the old
        "Name (Country)" form to the ISO -XX SUFFIX, so the namespace
        now has ONE convention and matches Ghana: Amazonas-VE,
        Bolívar-EC, Sucre-CO. The old spellings survive as
        geo-hierarchy aliases, and "Cocos (Keeling) Is." keeps its
        brackets because they are part of the real name of the
        territory, not a qualifier. A bare "Amazonas" or "Bolívar"
        tag still matches nothing on purpose. */
     ["Colombia", [
       "Amazonas-CO","Antioquia","Arauca",
       "Atlántico","Bogotá","Bolívar-CO",
       ...
       "Sucre-CO","Tolima","Valle del Cauca",
       "Vaupés","Vichada"
     ]],
     ["Costa Rica", [
     ...
     /* WEST AFRICA, 8.20.26 - Ivory Coast, Burkina Faso, Guinea
        and Liberia, completing the region beside Ghana and
        Nigeria/Benin. Each is a single level-3 unit.
        ⚠⚠ TWO OF THEM ARE BUILT AS UNIONS, ON PURPOSE. Natural Earth
        carries Burkina Faso's 45 PROVINCES and Guinea's 34
        PRÉFECTURES, but sources cite the 13 and 8 REGIONS, which are
        exact unions of them - so the regions are keyed and the
        smaller units never are. Ivory Coast and Liberia are 1:1.
        ⚠⚠ GUINEA HAS A PRÉFECTURE CALLED "MALI", the same string as
        the COUNTRY. Rolled into Labé region it is never keyed at all,
        so the collision disappears instead of needing a suffix - the
        "Niger State" problem, solved by the union rather than around
        it.
        ⚠ SIX KEYS ARE QUALIFIED because the bare name is ambiguous:
        Savanes-CI (Togo has one), Centre-BF / Est-BF / Nord-BF /
        Sud-Ouest-BF (Cameroon, Haiti, France, New Caledonia), and
        Maryland-LR against the US STATE. Burkina's COMPOUND
        directions (Centre-Est, Centre-Nord, ...) are unique
        worldwide and stay bare.
        ⚠ NE carries Ivory Coast's PRE-2011 régions; the 2011
        districts are not unions of them (Abidjan is a SUBSET of
        Lagunes), so they are not taggable - see
        ivorycoast-burkina-l4-table.json notTaggable. */
     /* TOGO + SIERRA LEONE + MALI + SENEGAL, 8.20.26 - closing West
        Africa. All four are single level-3 units and all four are 1:1
        with Natural Earth: NE already carries the tier sources cite,
        so unlike Burkina Faso and Guinea no unions were needed.
        ⚠⚠ SIERRA LEONE IS THE WORST COLLISION CASE HERE: ALL FOUR of
        its provinces are bare directions, each shared with 5-9
        admin_1 units worldwide (Eastern 8, Northern 7, Southern 5,
        Western 9). All four take the suffix; there is no judgement
        call - a bare "Western" belongs to nobody.
        ⚠⚠ TOGO'S "Savanes" IS WHY IVORY COAST'S IS "Savanes-CI".
        Both countries have one, so BOTH are qualified and the bare
        string deliberately matches nothing. Same for Centre (Haiti,
        Cameroon, Burkina) and Plateaux (Republic of the Congo).
        ⚠ MALI AND SENEGAL COLLIDE WITH NOTHING - all 23 of their
        units are unique worldwide and stay bare. The suffix is for
        ambiguity, not for tidiness.
        ⚠ NE anglicises Tombouctou as "Timbuktu"; the key takes the
        official spelling and both resolve. */
     /* CENTRAL AFRICA, 8.20.26 - WGSRPD West-Central Tropical
        Africa, 90 units, all 1:1 with Natural Earth.
        ⚠⚠ THE ISLANDS DO NOT PARENT TO THEIR COUNTRY. WGSRPD puts
        Bioko, Annobón, São Tomé and Príncipe in one level-3 unit,
        "Gulf of Guinea Islands", while "Equatorial Guinea" is the
        Río Muni MAINLAND only - so those five sit under the islands
        here even though Bioko and Annobón are Equatorial Guinean.
        ⚠⚠ CAMEROON NEEDS EIGHT SUFFIXES, the most after Sierra
        Leone: Centre, Est, Littoral, Nord, Nord-Ouest, Ouest, Sud
        and Sud-Ouest all belong to somebody else too. Only Adamaoua
        and Extrême-Nord are unique worldwide.
        ⚠⚠ AND THAT MOVED A LIVE KEY: Benin's bare "Littoral" became
        "Littoral-BJ", because two countries sharing a name means
        BOTH are qualified. Nothing was tagged with it.
        ⚠ "Litoral" (Equatorial Guinea, ONE t) is a different unit
        from "Littoral" (two t). They do not fold together.
        ⚠ DR Congo: NE has the PRE-2015 eleven provinces; the 2015
        reform SPLIT them into 26, so the modern names are SUBSETS
        and cannot be built. Bas-Congo -> Kongo Central was a rename
        with the same boundary and resolves as a variant. */
     /* EAST AFRICA, 8.20.26 - Kenya, Uganda, Chad, Eritrea,
        Somalia and Sudan, all 1:1 with Natural Earth. Tanzania and
        Ethiopia were already here and are untouched.
        ⚠⚠ UGANDA'S "Mbarara" IS ABSENT ON PURPOSE. Natural Earth
        carries TWO features of that name - one County, one District
        - covering DIFFERENT ground, and a name resolves to ONE
        feature, so keying it would silently ship half the district.
        A record cited to Mbarara resolves at country level instead.
        ⚠ FIVE KEYS ARE QUALIFIED: Kenya's Central/Eastern/Western
        and Sudan's Northern are bare directions belonging to 8-9
        admin_1 units worldwide, and Somalia's Bari is also a
        province of ITALY.
        ⚠ "Central Province" and "Western Province" are NOT aliases
        for Kenya's - they are Papua New Guinea's REAL KEYS. */
     ["Kenya", [
       "Central-KE","Coast","Eastern-KE",
       "Nairobi","North-Eastern","Nyanza",
       "Rift Valley","Western-KE"
     ]],
     ["Uganda", [
       "Abim","Adjumani","Agago",
       "Alebtong","Amolatar","Amudat",
       "Amuria","Amuru","Apac",
       "Arua","Budaka","Bududa",
       "Bugiri","Buhweju","Buikwe",
       "Bukedea","Bukomansimbi","Bukwa",
       "Bulambuli","Buliisa","Bundibugyo",
       "Bushenyi","Busia","Butaleja",
       "Butambala","Buvuma","Buyende",
       "Dokolo","Gomba","Gulu",
       "Hoima","Ibanda","Iganga",
       "Isingiro","Jinja","Kaabong",
       "Kabale","Kabarole","Kaberamaido",
       "Kalangala","Kaliro","Kalungu",
       "Kampala","Kamuli","Kamwenge",
       "Kanungu","Kapchorwa","Kasese",
       "Katakwi","Kayunga","Kibale",
       "Kiboga","Kibuku","Kiryandongo",
       "Kisoro","Kitgum","Koboko",
       "Kole","Kotido","Kumi",
       "Kween","Kyankwanzi","Kyegegwa",
       "Kyenjojo","Lamwo","Lira",
       "Luuka","Luweero","Lwengo",
       "Lyantonde","Manafwa","Maracha",
       "Masaka","Masindi","Mayuge",
       "Mbale","Mitooma","Mityana",
       "Moroto","Moyo","Mpigi",
       "Mubende","Mukono","Nakapiripirit",
       "Nakaseke","Nakasongola","Namayingo",
       "Namutumba","Napak","Nebbi",
       "Ngora","Ntoroko","Ntungamo",
       "Nwoya","Otuke","Oyam",
       "Pader","Pallisa","Rakai",
       "Rubirizi","Rukungiri","Sembabule",
       "Serere","Sheema","Sironko",
       "Soroti","Tororo","Wakiso",
       "Yumbe","Zombo"
     ]],
     ["Chad", [
       "Barh El Gazel","Batha","Borkou",
       "Chari-Baguirmi","Ennedi","Guéra",
       "Hadjer-Lamis","Kanem","Lac",
       "Logone Occidental","Logone Oriental","Mandoul",
       "Mayo-Kebbi Est","Mayo-Kebbi Ouest","Moyen-Chari",
       "Ouaddaï","Salamat","Sila",
       "Tandjilé","Tibesti","Ville de N'Djamena",
       "Wadi Fira"
     ]],
     ["Eritrea", [
       "Anseba","Debub","Debubawi Keyih Bahri",
       "Gash Barka","Maekel","Semenawi Keyih Bahri"
     ]],
     ["Somalia", [
       "Bakool","Banaadir","Bari-SO",
       "Bay","Galguduud","Gedo",
       "Hiiraan","Jubbada Dhexe","Jubbada Hoose",
       "Mudug","Nugaal","Shabeellaha Dhexe",
       "Shabeellaha Hoose"
     ]],
     ["Sudan", [
       "Blue Nile","Central Darfur","Eastern Darfur",
       "Gedarif","Gezira","Kassala",
       "Khartoum","North Darfur","North Kordufan",
       "Northern-SD","Red Sea","River Nile",
       "Sennar","South Kordufan","Southern Darfur",
       "Western Darfur","White Nile"
     ]],
     ["Cameroon", [
       "Adamaoua","Centre-CM","Est-CM",
       "Extrême-Nord","Littoral-CM","Nord-CM",
       "Nord-Ouest-CM","Ouest-CM","Sud-CM",
       "Sud-Ouest-CM"
     ]],
     ["Gabon", [
     ...
     ["Congo", [
       ...
       "Cuvette-Ouest","Kouilou","Likouala",
       "Lékoumou","Niari","Plateaux-CG",
       "Pointe-Noire","Pool","Sangha"
     ]],
     ["DR Congo", [
       "Bandundu","Bas-Congo","Kasaï-Occidental",
       "Kasaï-Oriental","Katanga","Kinshasa",
       "Maniema","Nord-Kivu","Orientale",
       "Sud-Kivu","Équateur"
     ]],
     ["Central African Republic", [
       "Bamingui-Bangoran","Bangui","Basse-Kotto",
       "Haut-Mbomou","Haute-Kotto","Kémo",
       "Lobaye","Mambéré-Kadéï","Mbomou",
       "Nana-Grébizi","Nana-Mambéré","Ombella-M'Poko",
       "Ouaka","Ouham","Ouham-Pendé",
       "Sangha-Mbaéré","Vakaga"
     ]],
     ["Equatorial Guinea", [
       "Centro Sur","Kié-Ntem","Litoral",
       "Wele-Nzás"
     ]],
     ...
     ["Burundi", [
       "Bubanza","Bujumbura Mairie","Bujumbura Rural",
       "Bururi","Cankuzo","Cibitoke",
       "Gitega","Karuzi","Kayanza",
       "Kirundo","Makamba","Muramvya",
       "Muyinga","Mwaro","Ngozi",
       "Rutana","Ruyigi"
     ]],
     ["Rwanda", [
       "Eastern-RW","Kigali","Northern-RW",
       "Southern-RW","Western-RW"
     ]],
     ["Togo", [
       "Centre-TG","Kara","Maritime",
       "Plateaux-TG","Savanes-TG"
     ]],
     ["Sierra Leone", [
       "Eastern-SL","Northern-SL","Southern-SL",
       "Western-SL"
     ]],
     ["Mali", [
       "Bamako","Gao","Kayes",
     ...
     ["Senegal", [
       "Dakar","Diourbel","Fatick",
       "Kaffrine","Kaolack","Kédougou",
       "Kolda","Louga","Matam",
       "Saint-Louis","Sédhiou","Tambacounda",
       "Thiès","Ziguinchor"
     ]],
     ["Ivory Coast", [
       "Agnéby","Bafing","Bas-Sassandra",
       "Cavally","Comoé","Denguélé",
       "Dix-Huit Montagnes","Fromager","Haut-Sassandra",
       "Lacs","Lagunes","Marahoué",
       "N'zi-Comoé","Savanes-CI","Sud-Bandama",
       "Sud-Comoé","Vallée du Bandama","Worodougou",
       "Zanzan"
     ]],
     ["Burkina Faso", [
       "Boucle du Mouhoun","Cascades","Centre-BF",
       "Centre-Est","Centre-Nord","Centre-Ouest",
       "Centre-Sud","Est-BF","Hauts-Bassins",
       "Nord-BF","Plateau-Central","Sahel",
       "Sud-Ouest-BF"
     ]],
     ["Guinea", [
     ...
     ["Benin", [
       ...
       "Donga","Mono",
       "Littoral-BJ","Ouémé","Plateau Department",
       "Zou"
     ]],
     ["Nigeria", [
     ...
     /* SARAWAK + SABAH DIVISIONS, 8.20.26. Borneo now carries FOUR
        depths: island -> state -> division. Natural Earth has no
        admin-2 anywhere, so the whole island was re-sourced onto one
        geoBoundaries layer - mixing sources disagreed along the
        Sarawak|Kalimantan frontier by a median 0.32 km with 36.6% of
        vertices past the simplify tolerance.
        ⚠ Divisions are UNIONS of districts (Sarawak 40, Sabah 27);
        the districts are never keyed. A division and its seat district
        share a name - Kapit Division contains Kapit District - and the
        division takes it.
        ⚠ West Coast and Interior are qualified -SB: bare, generic and
        ambiguous the moment anywhere else is divided that way. */
     ["Sarawak", [
       "Betong","Bintulu","Kapit",
       "Kuching","Limbang","Miri",
       "Mukah","Samarahan","Sarikei",
       "Serian","Sibu","Sri Aman"
     ]],
     ["Sabah", [
       "Interior-SB","Kudat","Sandakan",
       "Tawau","West Coast-SB"
     ]],
     ...
     /* v85 (8.18.26): THE BRAZIL/MEXICO STATES — 59 level-4 units,
        ...
        exactly as written: Amazonas-BR, Distrito Federal-BR, Mexico City (the NE vintage's Distrito Federal),
     ...
     ["Brazil West-Central", [
       "Distrito Federal-BR","Goiás","Mato Grosso","Mato Grosso do Sul"
     ]],
     ["Mexico Northwest", [
     ...
     /* v137: TAIWAN's 21 municipalities + MADAGASCAR's 6 provinces and
        22 regions — the shapes shipped in the HD sidecar 8.19–8.21 but
        never reached this table, so the generator could not promote them
        from ADDITIONAL DISTRIBUTION and the card had no hierarchy for
        suppression. Mirrored verbatim from geo-hierarchy's parents. */
     ["Taiwan", [
       "Changhua","Chiayi","Chiayi City","Hsinchu","Hsinchu City",
       "Hualien","Kaohsiung","Keelung","Kinmen","Miaoli","Nantou",
    ...
    /* v99: SUPPRESSION WALKS THE PUBLISHED TREE FIRST.
       It read SUBPARENT alone, so it only suppressed a parent the CARD's
       own table knew about - and that table does not carry Tanzania's
       regions. A. impressus, tagged Tanzania + Malawi + Ruvuma, lit the
       WHOLE of Tanzania beside its own Ruvuma region: the exact
       two-resolutions-at-once claim true coverage exists to prevent.
       Same migration the draw filter had in v96 and the wash in v93;
       SUBPARENT stays as the fallback for a failed hierarchy fetch. */
    var HIER = (data && data.hier && data.hier.places) || {};
    function upOf(n){
      /* v129: THE COUNTRY IS AN ANCESTOR TOO. This walked `parent`
         alone, and on Borneo the parent chain is geographic - Sarawak's
         parent is BORNEO, not Malaysia - so a post tagged Malaysia +
         Sarawak kept the whole country at full brightness beside its
         own subunit (A. brachyphyllus, measured live). Indonesia only
         ever got suppressed through a hand-kept SUBPARENT pair. The
         `country` field is the same claim at the same coarseness, so
         it joins the walk; the wash still picks its minimal cover
         AFTERWARDS, which is why demoted Malaysia does not wash the
         peninsula (the sarawakensis rule, untouched). */
      var t = HIER[n];
      if (t){
        var ups = [];
        if (t.parent) ups.push(t.parent);
        if (t.country && t.country !== n && t.country !== t.parent) ups.push(t.country);
    ...
    hits.concat(unmapped).forEach(function(n){
      var stack = upOf(n).slice(), guard = 0;
      while (stack.length && guard++ < 50){
        var up = stack.pop();
        if (suppress[up]) continue;
        suppress[up] = 1;
        upOf(up).forEach(function(x){ stack.push(x); });
      }
    });
    ...
    var parents = [];
    /* v146: places the post asserted WITHOUT a "?" - they can never
       inherit doubt from what they contain */
    var certainTag = {};
    hits.concat(unmapped).forEach(function(n){ if (!doubtful[n]) certainTag[n] = 1; });
    if (Object.keys(suppress).length){
      ...
      Object.keys(suppress).forEach(function(p){
        /* v146: the doubt is DEMOTED WITH THE PLACE, not thrown away.
           A suppressed parent must not paint as a doubtful LIT shape -
           that is why this delete exists - but whether the post said
           "Guinea?" or "Guinea" still decides what its container pill
           and its wash may inherit below. */
        if (!doubtful[p]) certainTag[p] = 1;
    ...
    if (Object.keys(HIER).length){
      ...
      washFrom.forEach(function(h){
        ...
        /* v98: THE COUNTRY FALLBACK, AND WHERE IT DOES NOT APPLY.
           A lit place with no drawable ancestor falls back to washing its
           COUNTRY - the v69 behaviour, and right for a mainland region:
           Assam lit inside a washed India reads correctly, and the reader
           asked to keep that (8 posts).
           It is WRONG when the country is not contiguous with the region,
           because the wash then shades land the record has nothing to do
           with. Three shapes of that:
             - an ISLAND or island group inside a much larger country -
               Maluku washed the whole of Indonesia including Borneo and
               Java (alocasia-brancifolia), Andaman Islands washed the
               Indian subcontinent, Hainan washed China;
             - a country SPLIT across sea - Peninsular Malaysia would wash
               Sabah and Sarawak 1,600 km away, the very complaint the v69
               island-beats-country rule was written for;
             - an overseas territory sharing a country name.
           ⚠ This is a hand list because the tree carries no island flag.
           It covers the 14 of 31 level-3 units that hit the fallback at
           all, so it is complete TODAY - a new island-like level-3 place
           needs adding here, and the right long-term home is a flag in
           geo-hierarchy.json rather than this file. */
        var NO_COUNTRY_WASH = {
          "Hainan":1, "Tasmania":1, "Andaman Islands":1, "Nicobar":1,
          "Lakshadweep":1, "Java":1, "Maluku":1, "Sulawesi":1, "Sumatra":1,
          "Bismarck Archipelago":1, "Nansei-shoto":1, "Puerto Rico":1,
          "San Andrés y Providencia":1, "Peninsular Malaysia":1
        };
        if (!found && !NO_COUNTRY_WASH[h]){
    ...
    /* ============ v146 — A CONTAINER OF ONLY DOUBTS IS DOUBTFUL =======
       Grower ask, on Amorphophallus elliotii: the post records Sierra
       Leone plainly and "Guinea?", "Mamou?", "Kindia?" - every Guinean
       record a question - yet Guinea read as an ordinary container,
       flat pill and plain wash, because demotion had stripped its "?"
       and a container has never carried doubt of its own. So the card
       stated a presence in Guinea that the post does not claim.
       A container inherits the hatch when EVERY place under it is
       doubtful AND the post never asserted it plainly: a country
       tagged "Guinea" beside a doubtful province stays certain, which
       is the whole reason certainTag exists.
       ⚠ Inheritance only - `doubtful` itself is untouched, so nothing
       here reaches the climate envelope, the ramp or the lit set. */
    var doubtAll = {};
    (function(){
      var lit = hits.concat(unmapped);
      if (!lit.length) return;
      var cand = parents.concat(autoCountries, wash || []);
      cand.forEach(function(c){
        if (doubtAll[c] || certainTag[c]) return;
        var inside = lit.filter(function(n){
          if (n === c) return false;
          var cur = n, guard = 0;
          while (cur && guard++ < 8){
            var h = HIER[cur] || {};
            if (h.country === c || h.parent === c) return true;
            cur = h.parent;
          }
          return false;
        });
        if (inside.length && inside.every(function(n){ return doubtful[n]; })) doubtAll[c] = 1;
      });
    })();
    return { hits:hits, continents:continents, unmapped:unmapped,
             ...
             autoCountries:autoCountries, doubtAll:doubtAll };
  }

  var apscHatchSeq = 0;
  function buildMap(data, hits, doubtful, wash, subparent, doubtAll){
    ...
    /* v140 (8.25.26): THE THIRD DEPTH IS EVERY DEEP CHAIN, not Borneo
       alone. Moulvibazar - Sylhet - Bangladesh rendered its country as
       plain ground while Kuching - Sarawak - Borneo washed the island
       (grower: "the shape of Sylhet is unknown to most - Bangladesh at
       lightest, Sylhet next, Moulvibazar brightest... we should have it
       for all multi level views"). The hand list is gone: any ancestor
       on the chain that is neither lit nor already the wash joins the
       lightest layer. The v69 ruling (chaii: Malaysia and Indonesia
       stay pills, the ISLAND washes) survives untouched, because up()
       follows the hierarchy's own parent links - Sarawak's chain stops
       at Borneo and never offers a country.
       ⚠ ISLANDS itself STAYS: the label engine still reads it (islands
       label first, and an island label may sit over open water) - only
       the third-depth gate stopped consulting it. Removing the
       declaration killed the whole card render silently on the first
       cut - it is referenced 40 lines further down. */
    var ISLANDS = { "Borneo": 1 };
    var HIER3 = (data && data.hier && data.hier.places) || {};

    /* v123: the bbox of a lit shape, read off the PATH DATA rather than
       getBBox(). getBBox() needs the element laid out, and these are
       built before the svg is mounted - it would return zeros. The feed
       writes paths as "M-0.17 -11.13L0.02 -11.06...", so a plain scan
       for numeric runs is enough and there is no regex to mangle. */
    var PATHBB = {}, BASEBB = [], SHAPEEL = {};
    function bboxOfPath(d){
      var x0=1e9,y0=1e9,x1=-1e9,y1=-1e9,buf="",vals=[],i,ch;
      for (i=0;i<=d.length;i++){
        ch = i<d.length ? d.charAt(i) : " ";
        if ((ch>="0"&&ch<="9")||ch==="."){ buf+=ch; }
        else if (ch==="-"){ if(buf){vals.push(parseFloat(buf));} buf="-"; }
        else { if(buf){vals.push(parseFloat(buf)); buf="";} }
      }
    ...
    (function(){
      ...
      for (var i = 0; i < start.length; i++){
        ...
        while (stack.length && guard++ < 30){
          ...
          if (seen[u]) continue;
          seen[u] = 1;
          if (!ctxSet[u] && hits.indexOf(u) < 0) islandSet[u] = 1;
    ...
    /* ⚠⚠ v146: THE SOFT HATCH IS SIZED TO THE SHAPE, NOT THE MAP. The
       doubtful hatch above is a fixed 2.4-unit tile in MAP DEGREES,
       which is fine on a province but grotesque on a country: framed on
       Guinea and Sierra Leone the card spans ~10 degrees, so a 2.4
       tile drew four enormous bands across the whole frame instead of a
       texture (measured, and it looked exactly as bad as it sounds).
       A container is by definition big, so its tile is a fraction of
       ITS OWN width - about twenty stripes across whatever it is,
       whether that is Guinea or Brazil - and the stroke keeps the same
       fraction of the tile so the weight never changes either. */
    var softSeq = 0;
    function softHatch(bb){
      var w = Math.max(0.08, (bb[2] - bb[0]) / 20);
      var id = hatchId + "s" + (++softSeq);
      var sp = document.createElementNS(NS, "pattern");
      sp.setAttribute("id", id);
      sp.setAttribute("patternUnits", "userSpaceOnUse");
      sp.setAttribute("width", String(w));
      sp.setAttribute("height", String(w));
      sp.setAttribute("patternTransform", "rotate(45)");
      var sl = document.createElementNS(NS, "line");
      sl.setAttribute("x1", "0"); sl.setAttribute("y1", "0");
      sl.setAttribute("x2", "0"); sl.setAttribute("y2", String(w));
      sl.setAttribute("stroke", "#afc090");
      sl.setAttribute("stroke-width", String(w * 0.3));
      sl.setAttribute("stroke-opacity", ".5");
      sp.appendChild(sl);
      defs.appendChild(sp);
      return id;
    }
    ...
    /* v96: ONE HIERARCHY. The subunit filter used the card's SUBPARENT
       table while the wash (v93) used the published tree, so anything the
       tree knows and SUBPARENT does not slipped through the filter
       entirely: a PHILIPPINE species was drawing 27 TANZANIAN regions.
       Parentage now comes from geo-hierarchy first, with SUBPARENT kept
       only as the fallback for a card whose hierarchy fetch failed. */
    var TREE = (data && data.hier && data.hier.places) || {};
    var MULTI = {};
    ((data && data.hier && data.hier.multiCountry) || []).forEach(function(m){ MULTI[m] = 1; });
    ...
    hits.forEach(function(h){
      ...
      parentsOf(h).forEach(function(par){ OPEN[par] = 1; });
    });

    /* v97: THE BASE LAYER IS ONE GROUP, AND THE OPACITY LIVES ON THE GROUP.
       Unlit ground was painted per-shape at 5.5% alpha, so wherever two
       base shapes covered the same land the fills STACKED to ~10.7% and the
       overlap read as a third, brighter tone. That is why Borneo looked lit
       beside Java, and why Labrador, Sicilia, Sinai, Kriti, Corse, Assam
       and Sudan glowed on a species that occurs in none of them - every one
       of them sits inside a country that also draws.
       An SVG group is FLATTENED BEFORE its opacity is applied, so opaque
       children inside a translucent group cannot stack no matter how they
       overlap. Group .10 x fill .55 = .055 and group .10 x stroke 1 = .10,
       which is pixel-identical to the old look for a SINGLE shape.
       Chosen over flattening the colour to an opaque hex because the card
       sits on rgba(19,26,21,.55) over rgb(11,18,13) - a hardcoded value
       would be wrong the moment the panel changes. */
    /* ⚠ TWO groups, not one. A group flattens before its opacity applies,
       but only OPAQUE children cannot stack - a first cut left them at
       .55 alpha and they still stacked inside the group (Borneo measured
       rgb(34,40,35) against Mongolia's rgb(28,34,30)). Opaque children
       force one alpha per group, and fill (.055) and stroke (.10) need
       different ones, so they get a group each. */
    var gFill = document.createElementNS(NS, "g");
    gFill.setAttribute("class", "apsc-baselayer-f");
    svg.appendChild(gFill);
    var gLine = document.createElementNS(NS, "g");
    gLine.setAttribute("class", "apsc-baselayer-l");
    ...
    order.forEach(function(name){
      ...
      /* v97: THE v96 MULTI-COUNTRY SUPPRESSION IS REVERTED. It assumed the
         constituent countries covered the same ground, and for several they
         do not: Papua New Guinea is in the feed but was not being drawn, so
         hiding New Guinea left the eastern half of the island as a HOLE.
         Korea, Lebanon-Syria and the Baltic States carry the same risk.
         The glow those shapes caused was never really about overlap - it
         was ALPHA STACKING, and that is fixed properly in the base layer
         below, which costs no shapes at all. */
      var ps = parentsOf(name);
      if (ps.length && !onSet[name] && !ctxSet[name]){
      ...
      var _bb = bboxOfPath(d);
      /* v146: the context layers take the soft hatch when everything
         they contain is doubtful (ctx and ctx0 alike, so a country and
         an island both read it). Needs the bbox, so it sits here. */
      if (_bb && DALL[name] && !onSet[name] && (ctxSet[name] || islandSet[name])){
        p.style.fill = "url(#" + softHatch(_bb) + ")";
      }
      if (_bb){
        /* every shape's box feeds the open-water test; the lit and
           washed ones also feed placement and hover */
        BASEBB.push(_bb);
        if (onSet[name] || ctxSet[name] || islandSet[name]) PATHBB[name] = _bb;
      }
      /* EVERY shape, not just the lit ones - hovering a country name
         must light the whole country, and the country outline is an
         ordinary ground path */
      SHAPEEL[name] = p;
    ...
    /* ── POINT LOCALITIES (Alabat, Himalaya, Gilbert Is.) ───────────
       A place with no polygon draws as a MARKER: dot + leader + a
       PERMANENT label. v120 sized the dot correctly and thereby made
       it ~2px, which the grower fairly read as having removed it - a
       point record has to be as findable as a lit shape, and it is the
       only thing on the map that carries no shape to read. */
    hits.forEach(function(name){
      var dot = data.dots && data.dots[name];
      if (!dot) return;
      var g = document.createElementNS(NS,"g");
      g.setAttribute("class", "apsc-mark");
      g.setAttribute("data-x", dot[0]);
      g.setAttribute("data-y", -dot[1]);
      /* leader FIRST so the dot and the label paint over it */
      var ln = document.createElementNS(NS,"line");
      ln.setAttribute("class","apsc-mark__lead");
      g.appendChild(ln);
      var tx = document.createElementNS(NS,"text");
      tx.setAttribute("class","apsc-mark__label");
      tx.textContent = name;
      g.appendChild(tx);
      var c = document.createElementNS(NS,"circle");
      c.setAttribute("cx", dot[0]); c.setAttribute("cy", -dot[1]);
      /* r, the leader and the type size are all set by sizeMarks() */
    ...
    /* -- v123: ONE LABEL PER LIT COUNTRY --------------------------
       WARNING: ONLY WHEN MORE THAN ONE COUNTRY IS LIT. With a single
       country there is nothing to disambiguate and the label is pure
       noise, so every ordinary one-country card is left exactly as it
       was. This is aimed at the maps that actually fail: a West African
       species lit across seven countries at region level. */
    /* v133: WORLD ZOOMS OUT AROUND THE RANGE - it does not map the
       continent. v132 framed the continent's actual bbox and the
       grower caught both failure modes at once: Africa's box dragged
       johnsonii's range out of centre toward the top of the frame,
       and longiloba's range is WIDER than its continent's box, so
       "World" zoomed IN. The intent was always "continent-WIDE
       context": same centre as the framed range, 2.5x the width,
       capped at the full earth and clamped inside it. The lit area
       stays exactly where the reader's eye already is. */
    /* declared HERE so sizeDots and the buttons close over it, but
       COMPUTED just before the first sizeDots() call - this block runs
       before `zoom` exists, and reading it here killed the whole card
       (caught in the pre-deploy probe: no map rendered at all). */
    var WORLDVB = null;

    var CLAB = [], SLOTMEM = {};
    function clabHL(g, on){
      String(g.getAttribute("data-shapes") || "").split("|").forEach(function(n){
        var el = SHAPEEL[n];
        if (el && el.classList) el.classList[on ? "add" : "remove"]("apsc-hl");
      });
      /* the dim rides the same toggle - classList, never setAttribute:
         the svg may already carry apsc-tri and must keep it */
      if (svg.classList) svg.classList[on ? "add" : "remove"]("apsc-dim");
    }
    (function(){
      /* v126: TWO GROUPINGS.
         COUNTRY MODE (the usual): one label per lit country, and only
         when more than one country is lit - a single-country map has
         nothing to disambiguate.
         SHAPE MODE (the Borneo three-depth maps): when the island
         depth is in play the reader is looking at NESTED units, not
         countries - so label the lit unit(s) AND the island itself
         (grower request: baginda should read "East Kalimantan" and
         "Borneo"). The mid-depth wash stays unlabeled: three tiers of
         type on one small map is noise. */
      var entries = [];
      /* v130: ONE RULE EVERYWHERE - LABELS NAME CONTAINERS, NEVER WHAT
         IS HIGHLIGHTED. The lit shape's own name lives in the hover
         readout; a label repeating it is noise (grower: "no need to
         label what's highlighted"). Two kinds of container qualify:
         COUNTRIES, only when more than one is in play - one country is
         nothing to disambiguate, which is why commutatus (India alone)
         and zebrina (Philippines alone) carry no labels - and ISLANDS,
         always, however they arrived (island depth, wash or lit),
         because the three-depth maps exist precisely so the island
         reads as a unit. baginda and brachyphyllus now both say just
         "Borneo"; a multi-country Borneo post says the countries AND
         the island. */
      var islQ = {};
      Object.keys(ISLANDS).forEach(function(k){
        if (islandSet[k] || ctxSet[k] || onSet[k]) islQ[k] = 1;
      });
      var by = {};
      Object.keys(onSet).forEach(function(name){
        if (islQ[name]) return;          /* an island is not a country group */
        var bb = PATHBB[name]; if (!bb) return;
        var h = HIER3[name] || {};
        var c = h.country || (h.level === 3 ? name : null);
        if (!c) return;
        var b = by[c];
        if (!b) by[c] = { bb: bb.slice(), shapes: [name] };
        else {
          b.shapes.push(name);
          if(bb[0]<b.bb[0])b.bb[0]=bb[0]; if(bb[1]<b.bb[1])b.bb[1]=bb[1];
          if(bb[2]>b.bb[2])b.bb[2]=bb[2]; if(bb[3]>b.bb[3])b.bb[3]=bb[3];
        }
      });
      var cNames = Object.keys(by);
      if (cNames.length >= 2){
        cNames.forEach(function(c){
          /* the hover set leads with the COUNTRY's own polygon, so the
             reader sees the whole country, not just the lit pieces -
             then the lit subunits on top of it */
          entries.push({ text:c, bb:by[c].bb, shapes:[c].concat(by[c].shapes) });
        });
      }
      Object.keys(islQ).forEach(function(n){
        if (PATHBB[n]) entries.push({ text:n, bb:PATHBB[n], shapes:[n] });
      });
      if (!entries.length) return;
      if (entries.length > CFG.CLAB_MAX){
        /* v131: OVER THE CAP, TRIM THE BIGGEST AND KEEP THE SMALLEST
           (grower ruling). The cap used to drop EVERYTHING - so the
           14-country maps, where a reader most needs orientation, got
           no labels at all. And the labels a reader actually needs are
           the OBSCURE ones: China and India are recognisable from
           their outlines, it is Togo, Brunei and the Lesser Sundas
           that need naming. Islands are exempt from the trim - they
           are the reason the three-depth maps exist. Size is measured
           by the container's LIT extent, which is also what the label
           gets placed around. */
        var isl = entries.filter(function(e){ return ISLANDS[e.text]; });
        var rest = entries.filter(function(e){ return !ISLANDS[e.text]; });
        rest.sort(function(a,b){
          return (a.bb[2]-a.bb[0])*(a.bb[3]-a.bb[1]) - (b.bb[2]-b.bb[0])*(b.bb[3]-b.bb[1]);
        });
        entries = isl.concat(rest).slice(0, CFG.CLAB_MAX);
      }
      entries.forEach(function(e){
        var g = document.createElementNS(NS,"g");
        g.setAttribute("class","apsc-clab");
        g.setAttribute("data-bb", e.bb.join(" "));
        g.setAttribute("data-shapes", e.shapes.join("|"));
        var ln = document.createElementNS(NS,"line");
        ln.setAttribute("class","apsc-clab__lead");
        g.appendChild(ln);
        var tx = document.createElementNS(NS,"text");
        tx.setAttribute("class","apsc-clab__label");
        tx.textContent = e.text;
        /* hover on the NAME lights the shape(s) it names - the
           association question, answered on demand. pointer-events
           comes back on for just the text; the leader stays inert, and
           a pointerdown on a label still BUBBLES to the svg, so a drag
           that starts on a name pans the map like anywhere else. */
        tx.style.pointerEvents = "auto";
        g.appendChild(tx);
        g.addEventListener("mouseenter", function(){ clabHL(g, true); });
        g.addEventListener("mouseleave", function(){ clabHL(g, false); });
        svg.appendChild(g);
        CLAB.push(g);
      });
    })();
    ...
    function sizeDots(){
      ...
      /* ⚠⚠ SIZED IN REAL PIXELS, NOT IN VIEWBOX FRACTIONS. A fraction
         of the viewBox holds through ZOOM but NOT through a change of
         RENDERED SIZE, and this map has one: on scroll the follow panel
         takes .apsc--compact, which clamps the svg max-height to
         110-170px (88px on the narrow breakpoint). The same user-unit
         radius then renders at about half the width, so the marker
         shrank away exactly when the map got small enough to need it.
         (It was already happening at rest: max-height letterboxes the
         drawn map to ~236px inside its 420px box, which is why a
         "3px" radius was really measuring 1.7px.)

         getScreenCTM() reports the live user-space -> CSS-pixel scale
         and folds BOTH causes into one number - viewBox zoom AND
         rendered size. Divide the wanted pixel size by it and the
         marker holds through a zoom, a collapse and a window resize. */
      var W = vb[2];
      var ctm = svg.getScreenCTM && svg.getScreenCTM();
      /* ⚠ no CTM before the svg is in the document, and this runs once
         pre-mount. The ResizeObserver below re-runs it after mount, so
         the fallback only has to be harmless, not right. */
      var k = (ctm && ctm.a) ? ctm.a : ((svg.getBoundingClientRect().width || W) / W);
      if (!(k > 0)) k = 1;
      var px = function(v){ return v / k; };
      var rr   = px(CFG.MARK_R_PX);
      var lead = px(CFG.MARK_LEAD_PX);
      var fs   = px(CFG.MARK_FS_PX);
      [].slice.call(svg.querySelectorAll("g.apsc-mark")).forEach(function(g){
        var x = +g.getAttribute("data-x"), y = +g.getAttribute("data-y");
        var c = g.querySelector("circle"), ln = g.querySelector("line"),
            tx = g.querySelector("text");
        if (c) c.setAttribute("r", rr.toFixed(4));
        if (!tx) return;
        tx.setAttribute("font-size", fs.toFixed(4));
        /* ⚠ MEASURE THE LABEL, DO NOT GUESS IT. The first cut flipped on
           a fixed fraction of the viewBox, which is unrelated to how wide
           "Alabat" or "Gilbert Is." actually is - a long name still ran
           off the map. getComputedTextLength() returns USER UNITS, the
           same space the viewBox is in, so the comparison is exact. */
        var tw = 0;
        try { tw = tx.getComputedTextLength() || 0; } catch (e) { tw = String(tx.textContent||"").length * fs * 0.55; }
        if (!tw) tw = String(tx.textContent||"").length * fs * 0.55;
        var pad = fs * 0.6;
        var right = (x + lead + tw + pad) <= (vb[0] + W);
        /* if neither side fits, keep it inside on whichever has more room */
        if (!right && (x - lead - tw - pad) < vb[0]) right = ((vb[0] + W) - x) >= (x - vb[0]);
        var dx = right ? lead : -lead;
        var ex = x + dx, ey = y - lead * 0.55;
        ln.setAttribute("x1", x.toFixed(4)); ln.setAttribute("y1", y.toFixed(4));
        ln.setAttribute("x2", ex.toFixed(4)); ln.setAttribute("y2", ey.toFixed(4));
        tx.setAttribute("x", (ex + (right ? pad * 0.5 : -pad * 0.5)).toFixed(4));
        tx.setAttribute("y", (ey - fs * 0.25).toFixed(4));
        tx.setAttribute("text-anchor", right ? "start" : "end");
      });

      /* -- LABEL PLACEMENT: quiet ground + leaders + hysteresis ------
         v126, after grower direction: put the name on land that is NOT
         highlighted whenever possible, and bring the leader back to
         carry the association. So the cost model changed sides: a
         candidate now pays for covering ANY lit ground - its own
         feature included - a lighter charge for the mid-depth wash,
         and a small penalty for open water, because a name floating in
         the sea belongs to nothing. Association comes from adjacency
         when the label lands close, and from the LEADER when it had to
         move away.

         HYSTERESIS (the dynamic-labeling papers' core point): once a
         label holds a slot it KEEPS it while that slot stays close to
         optimal, instead of hopping to a marginally better one on
         every zoom step. It re-places only when its slot goes off-map,
         collides, or becomes clearly worse. */
      var cfs = px(CFG.CLAB_FS_PX), gap = px(CFG.CLAB_GAP_PX);
      /* v132: LABELS COME OFF AT THE WIDE VIEW (grower). At continent
         scale the names crowd into a smudge; the wide view exists to
         show WHERE ON EARTH, not which unit is which. Keyed to the
         world target's width, so wheel-zooming that far out behaves
         exactly like pressing WORLD. */
      if (vb[2] >= ((WORLDVB && WORLDVB[2]) || full[2]) * 0.95){
        [].slice.call(svg.querySelectorAll("g.apsc-clab")).forEach(function(g){ g.style.display = "none"; });
      } else {
      var rows = [];
      [].slice.call(svg.querySelectorAll("g.apsc-clab")).forEach(function(g){
        var bb = String(g.getAttribute("data-bb")).split(" ").map(Number);
        var tx = g.querySelector("text");
        g.style.display = "";
        tx.setAttribute("font-size", cfs.toFixed(4));
        var tw = 0;
        try { tw = tx.getComputedTextLength() || 0; } catch(e){ tw = 0; }
        if (!tw) tw = String(tx.textContent||"").length * cfs * 0.62;
        rows.push({ g:g, tx:tx, ln:g.querySelector("line"), bb:bb, tw:tw,
                    name: tx.textContent,
                    isIsland: !!islandSet[tx.textContent],
                    cx:(bb[0]+bb[2])/2, cy:(bb[1]+bb[3])/2,
                    area:(bb[2]-bb[0])*(bb[3]-bb[1]) });
      });

      /* what a candidate pays to cover */
      var priced = [];
      Object.keys(onSet).forEach(function(n){ if (PATHBB[n]) priced.push({ bb:PATHBB[n], w:1 }); });
      Object.keys(ctxSet).forEach(function(n){ if (PATHBB[n]) priced.push({ bb:PATHBB[n], w:0.35 }); });

      var placed = [];
      function overlapArea(r, b){
        var w = Math.min(r[2], b[2]) - Math.max(r[0], b[0]);
        var h = Math.min(r[3], b[3]) - Math.max(r[1], b[1]);
        return (w > 0 && h > 0) ? w * h : 0;
      }
      function rectClash(r){
        for (var i = 0; i < placed.length; i++){
          var q = placed[i];
          if (r[0] < q[2] && q[0] < r[2] && r[1] < q[3] && q[1] < r[3]) return true;
        }
        return false;
      }
      function costOf(r, isIsland){
        var c = 0;
        for (var i = 0; i < priced.length; i++){
          /* an ISLAND's own mid-depth wash is its own body - the island
             name sitting on it is association, not noise. It still pays
             full price for the LIT unit, which is how "BORNEO" stays
             off East Kalimantan. */
          if (isIsland && priced[i].w < 1) continue;
          c += overlapArea(r, priced[i].bb) * priced[i].w;
        }
        /* open water: bboxes overstate land (a coastline's box covers
           water too), so this is a nudge toward land, not a rule */
        var land = 0;
        for (var j = 0; j < BASEBB.length && !land; j++) if (overlapArea(r, BASEBB[j]) > 0) land = 1;
        if (!land) c += (r[2]-r[0]) * (r[3]-r[1]) * 0.25;
        return c;
      }
      function offMap(r){
        return r[0] < vb[0] || r[2] > vb[0] + W || r[1] < vb[1] || r[3] > vb[1] + vb[3];
      }

      var rankBias = cfs * cfs * 0.12;
      rows.sort(function(a,b){ return b.area - a.area; });
      rows.forEach(function(r){
        var bb = r.bb, tw = r.tw;
        var cands = [];
        /* the inside slot stays a candidate - when a country's interior
           is QUIET (Mali lighting only Kayes and Sikasso), the name
           inside it costs nothing and reads best of all. When the
           interior is lit, the cost model prices it out on its own. */
        if ((tw + cfs) <= (bb[2]-bb[0]) && (cfs*1.6) <= (bb[3]-bb[1])){
          cands.push({ x:r.cx, y:r.cy + cfs*0.35, a:"middle", inside:true, rank:0,
            rect:[r.cx - tw/2 - gap*0.5, r.cy - cfs*0.7, r.cx + tw/2 + gap*0.5, r.cy + cfs*0.55] });
        }
        /* QUADRANT slots: a large feature can hold its own name in a
           QUIET CORNER of itself when its centre is occupied - which is
           how "BORNEO" sits on the island instead of in the sea at the
           end of a leader that appeared to point at East Kalimantan.
           Inside placements, so no leader. Only for features with real
           room; on a small shape a quadrant is just the centre again. */
        var bw = bb[2]-bb[0], bh = bb[3]-bb[1];
        if (bw > tw*1.6 && bh > cfs*5){
          [[-1,-1],[1,-1],[-1,1],[1,1]].forEach(function(q,qi){
            var qx = r.cx + q[0]*bw/4, qy = r.cy + q[1]*bh/4;
            cands.push({ x:qx, y:qy + cfs*0.35, a:"middle", inside:true, rank:1+qi,
              rect:[qx - tw/2 - gap*0.5, qy - cfs*0.7, qx + tw/2 + gap*0.5, qy + cfs*0.55] });
          });
        }
        function ringAt(d, base){
          var E = bb[2] + d, Wx = bb[0] - d, N = bb[1] - d, S = bb[3] + d;
          return [
            { x:E,   y:N + cfs*0.9,    a:"start",  rank:base+1 },
            { x:Wx,  y:N + cfs*0.9,    a:"end",    rank:base+2 },
            { x:E,   y:S - cfs*0.1,    a:"start",  rank:base+3 },
            { x:Wx,  y:S - cfs*0.1,    a:"end",    rank:base+4 },
            { x:E,   y:r.cy + cfs*0.35, a:"start",  rank:base+5 },
            { x:Wx,  y:r.cy + cfs*0.35, a:"end",    rank:base+6 },
            { x:r.cx, y:N - cfs*0.15,   a:"middle", rank:base+7 },
            { x:r.cx, y:S + cfs*0.9,    a:"middle", rank:base+8 }
          ];
        }
        /* a near ring for quiet neighbourhoods and a far ring to
           ESCAPE crowded ones - the far ring is where the leader
           earns its keep */
        ringAt(gap, 4).concat(ringAt(gap*6, 12)).forEach(function(c){
          var x0 = c.a === "start" ? c.x : c.a === "end" ? c.x - tw : c.x - tw/2;
          c.rect = [x0 - gap*0.4, c.y - cfs*0.95, x0 + tw + gap*0.4, c.y + cfs*0.3];
          cands.push(c);
        });
        var best = null, prevBest = null, prevRank = SLOTMEM[r.name];
        for (var i = 0; i < cands.length; i++){
          var c = cands[i];
          if (offMap(c.rect) || rectClash(c.rect)) continue;
          c.score = costOf(c.rect, r.isIsland) + c.rank * rankBias;
          if (!best || c.score < best.score) best = c;
          if (prevRank !== undefined && c.rank === prevRank) prevBest = c;
        }
        if (!best){ r.g.style.display = "none"; return; }
        if (prevBest && prevBest.score <= best.score + cfs * cfs * 0.9) best = prevBest;
        SLOTMEM[r.name] = best.rank;
        placed.push(best.rect);
        r.g.setAttribute("class", best.inside ? "apsc-clab apsc-clab--in" : "apsc-clab");
        r.tx.setAttribute("x", best.x.toFixed(4));
        r.tx.setAttribute("y", best.y.toFixed(4));
        r.tx.setAttribute("text-anchor", best.a);
        if (r.ln && !best.inside){
          /* the leader draws on EVERY set-off label. The first rule
             keyed it to distance from the BBOX, and a bbox is not the
             shape: East Kalimantan's box spans half of Borneo, so its
             label sat over Sarawak, technically "touching" its own box,
             with no line to say which province it named. If the name
             is not INSIDE its area, it points. */
          var lx = best.a === "start" ? best.rect[0] + gap*0.4
                 : best.a === "end"   ? best.rect[2] - gap*0.4 : best.x;
          var ly = best.y - cfs*0.3;
          r.ln.style.display = "";
          r.ln.setAttribute("x1", lx.toFixed(4)); r.ln.setAttribute("y1", ly.toFixed(4));
          r.ln.setAttribute("x2", r.cx.toFixed(4)); r.ln.setAttribute("y2", r.cy.toFixed(4));
        } else if (r.ln) r.ln.style.display = "none";
      });
      }
    }
    ...
    if (bb){
      ...
      /* v136: ONE CLICK TIGHTER (grower, with screenshots): the padded
         frame consistently landed one +-step wider than where a reader
         actually starts reading, in the full panel AND in the pinned
         collapse - which share this viewBox, so one shrink fixes both.
         Shrink by exactly the zoom button's own factor about the same
         centre, floored at MAP_MIN_DEG. Everything derived from `zoom`
         - WORLD, MINW, the wide-view label cutoff, Close in - inherits
         the tighter frame automatically. */
      var tw = Math.max(w / 1.4, CFG.MAP_MIN_DEG);
      if (tw < w - 1e-9){ h *= tw / w; w = tw; }
      zoom = [cx - w/2, cy - h/2, w, h];
    }
    /* v133: WORLD = the framed range's own centre at 2.5x its width,
       capped at the full earth and clamped inside it. Not the
       continent's geometry - v132 framed the continent bbox and got
       both failure modes at once: Africa's box dragged johnsonii out
       of centre, and longiloba's range is WIDER than its continent
       box, so "World" zoomed IN. Context, not cartography. */
    (function(){
      /* ⚠⚠ THE VIEWBOX ASPECT IS THE ELEMENT'S SHAPE. The svg is
         height:auto, so its rendered height comes from the viewBox
         ratio - the per-axis fit produced a wider-flatter box and the
         MAP ELEMENT ITSELF shrank 360px -> 297px on longiloba, pushing
         the whole panel up. Grower: "clicking on world should
         absolutely not change the shape of anything on screen." So the
         wide view inherits the framed aspect EXACTLY - h is derived
         from w, never fitted on its own.

         Where the window then overruns the drawn world vertically, the
         overflow is BLANK - and blank is free: the ocean has no fill
         of its own, so empty band and open sea are the same dark.
         Horizontally the window still slides into the drawn world,
         because sideways void is NOT free - east of the dateline the
         Americas belong in frame and their absence would show. */
      var w = Math.min(zoom[2] * 2.5, full[2]);
      var h = w * zoom[3] / zoom[2];
      var x = zoom[0] + zoom[2] / 2 - w / 2;
      var y = zoom[1] + zoom[3] / 2 - h / 2;
      if (w < full[2] - 1e-9){
        if (x < full[0]) x = full[0];
        if (x + w > full[0] + full[2]) x = full[0] + full[2] - w;
      } else x = full[0] + (full[2] - w) / 2;
      if (h < full[3] - 1e-9){
        if (y < full[1]) y = full[1];
        if (y + h > full[1] + full[3]) y = full[1] + full[3] - h;
      } else y = full[1] + (full[3] - h) / 2;
      WORLDVB = [x, y, w, h];
    })();
    ...
    /* ⚠⚠ THE MAP CHANGES RENDERED SIZE WITHOUT THE VIEWBOX CHANGING.
       sizeDots() was wired to setVB() only, which catches zoom and pan
       but NOT the scroll-collapse (a CSS max-height transition) or a
       window resize - in both the viewBox is untouched while the
       pixels-per-degree halves. Observing the svg catches every cause
       at once, including the first post-mount layout, which is also
       the run that finally gets a real getScreenCTM. */
    if (window.ResizeObserver){
      try { new ResizeObserver(function(){ sizeDots(); }).observe(svg); }
      catch (e) {}
    }
    ...
    function show(name, lit, doubt){
      ...
      /* v128: the readout names the CONTAINING unit too - "Tombouctou"
         alone assumes the reader knows their Malian regions. One hop up
         the published tree: a subunit shows its parent, a country (no
         parent) shows itself alone. */
      /* v129: the WHOLE chain, ending at the country - grower ask:
         West Kalimantan should read "West Kalimantan - Kalimantan -
         Borneo - Indonesia". Walk parent links up (cycle-guarded),
         then close with `country` if the chain has not already
         reached it. A country names itself alone. */
      var chain = [], seen = {}, cur = name, hops = 0;
      seen[name] = 1;
      while (hops++ < 5){
        var t2 = HIER3[cur] || {};
        if (!t2.parent || seen[t2.parent]) break;
        chain.push(t2.parent); seen[t2.parent] = 1; cur = t2.parent;
      }
      var ctry = (HIER3[name] || {}).country;
      if (ctry && !seen[ctry]) chain.push(ctry);
      var tail = "";
      for (var ci = 0; ci < chain.length; ci++){
        tail += "<i>&nbsp;&ndash;&nbsp;" + esc(chain[ci]) + "</i>";
      }
      readout.innerHTML = esc(name) + tail +
        (lit ? (doubt ? "<em>reported &mdash; doubtful</em>" : "<em>recorded here</em>") : "");
      readout.setAttribute("data-on","1");
    }
    /* v128: THE REVERSE HOVER. The labels light their shapes; hovering
       a shape now lights its label back. A shape in some label's hover
       set uses that label; a shape with no label of its own (an unlit
       piece, a mid-depth wash) walks UP the tree until a label answers
       to an ancestor's name - Tombouctou has no label, Mali does. */
    function ownerLabel(n){
      if (!n) return null;
      var gs = [].slice.call(svg.querySelectorAll("g.apsc-clab"));
      for (var i = 0; i < gs.length; i++){
        var ds = "|" + (gs[i].getAttribute("data-shapes") || "") + "|";
        if (ds.indexOf("|" + n + "|") >= 0) return gs[i];
      }
      var seen = {}, cur = n, hops = 0;
      while (cur && hops++ < 4 && !seen[cur]){
        seen[cur] = 1;
        var h = HIER3[cur] || {};
        var up = h.parent || h.country;
        if (!up || up === cur) return null;
        for (var j = 0; j < gs.length; j++){
          var t = gs[j].querySelector("text");
          if (t && t.textContent === up) return gs[j];
        }
        cur = up;
      }
      return null;
    }
    /* v135: the chips live OUTSIDE this closure but want the same
       highlight the labels have. One hook on the returned node; dim
       only when there is actually a shape to leave lit. */
    wrap.__hl = function(n, on){
      var elx = SHAPEEL[n];
      if (!elx || !elx.classList) return;
      elx.classList[on ? "add" : "remove"]("apsc-hl");
      if (svg.classList) svg.classList[on ? "add" : "remove"]("apsc-dim");
    };
    var hotLab = null;
    function labelHot(n){
      var g = ownerLabel(n);
      if (g === hotLab) return;
      if (hotLab && hotLab.classList) hotLab.classList.remove("apsc-clab--hot");
      hotLab = g;
      if (hotLab && hotLab.classList) hotLab.classList.add("apsc-clab--hot");
    }
    svg.addEventListener("mouseover", function(e){
      ...
      var nm = nameAt(e.target);
      show(nm, /\bapsc-on\b/.test(cls) || /\bapsc-dot\b/.test(cls),
           /apsc-on--doubtful|apsc-dot--doubtful/.test(cls));
      labelHot(nm);
    });
    svg.addEventListener("mouseleave", function(){ show(null); labelHot(null); });
    /* touch: a tap names the region instead of doing nothing */
    svg.addEventListener("click", function(e){
      var p = e.target.closest ? e.target.closest("path") : null;
      var cls = p ? (p.getAttribute("class") || "") : "";
      var nm = nameAt(e.target);
      show(nm, /\bapsc-on\b/.test(cls), /apsc-on--doubtful/.test(cls));
      labelHot(nm);
    });
    ...
    if (bb){
      ...
      function setVB(){
        ...
        /* v27: the world-context hairline is 0.15-degree-simplified -
           past ~2.5x zoom its long straight segments diverge from the
           detailed coastlines and read as stray chords (grower
           screenshot, Divisions over Cambodia). Context is an
           overview idea; it fades where it stops being true. */
        svg.classList.toggle("apgm--zoomed", CUR[2] < full[2] / 2.5);
        sizeDots(); syncBtns();
      }
      ...
      worldBtn.addEventListener("click", function(){
        wide = !wide;
        CUR = (wide ? (WORLDVB || full) : zoom).slice();
  ...
  function render(mount, parsed, facts, data){
    ...
    var mapNode = isCultivarPost ? null
                : buildMap(data, geo.hits, geo.doubtful, geo.wash, geo.subparent, geo.doubtAll);
    ...
    if (geo.hits.length || geo.continents.length || geo.unmapped.length ||
      ...
      /* v135: CHAINS, NOT A BAG. Every place chip is grouped under its
         container (the labels' own rule: `country`, or itself at level
         3), container pill first, subunits linked to it by a connector
         line. Inside a chain a subunit DISPLAYS without its
         disambiguation suffix - "Savanes-CI" is just "Savanes" when it
         visibly hangs off Ivory Coast; the suffix exists to
         disambiguate and the chain already has. The TAG (href) keeps
         the full spelling. Hovering any pill lights its shape on the
         map through the same dim the labels use. */
      var HP = (data && data.hier && data.hier.places) || {};
      function containerOf(n){
        var h = HP[n] || {};
        return h.country || (h.level === 3 ? n : null);
      }
      var chains = {}, order = [];
      function intoChain(name, node){
        var c = containerOf(name) || name;
        if (!chains[c]){ chains[c] = { head: null, members: [] }; order.push(c); }
        if (name === c) chains[c].head = node;
        else chains[c].members.push(node);
      }
      function wireHover(node, name){
        node.addEventListener("mouseenter", function(){
          if (mapNode && mapNode.__hl) mapNode.__hl(name, true);
        });
        node.addEventListener("mouseleave", function(){
          if (mapNode && mapNode.__hl) mapNode.__hl(name, false);
        });
      }
      function place(name, cls, tag, title){
        var isSub = containerOf(name) && containerOf(name) !== name;
        var label = isSub ? name.replace(/-[A-Z]{2}$/, "") : name;
        var node = chip(label, cls, tag || name, title);
        node.__place = name;          /* v144: the chain walk needs the key */
        wireHover(node, name);
        intoChain(name, node);
      }
      /* v146: a container holding nothing but doubts wears the hatch
         itself, and says why in its title */
      var dAll = geo.doubtAll || {};
      function pcls(n){ return "apsc-chip--parent" + (dAll[n] ? " apsc-chip--doubtful" : ""); }
      function pttl(n, base){
        return dAll[n] ? n + " — every record here is reported as doubtful" : base;
      }
      (geo.autoCountries || []).forEach(function(c){
        place(c, pcls(c), c, pttl(c,
          c + " — the country containing the recorded range (not tagged on this post)"));
      });
      (geo.parents || []).forEach(function(p){
        place(p, pcls(p), p, pttl(p,
          p + " — contains the recorded range below"));
      });
      geo.hits.forEach(function(h){
        if (geo.doubtful[h]){
          place(h, "apsc-chip--doubtful", geo.doubtfulTag[h] || h,
      ...
      geo.unmapped.forEach(function(u){
        ...
        place(u, ucls, geo.doubtful[u] ? (geo.doubtfulTag[u] || u) : u, uttl);
      });
      order.forEach(function(c){
        var ch = chains[c];
        /* a chain with members but no head: the container was never
           tagged and not auto-added - lead with a quiet parent pill so
           the chain still names its country */
        if (!ch.head && ch.members.length > 1){
          ch.head = chip(c, "apsc-chip--parent", c,
            c + " — the country containing the places below");
          wireHover(ch.head, c);
        }
        if (!ch.head){ if (ch.members[0]) distBox.appendChild(ch.members[0]); return; }
        var run = el("span", "apsc-chain");
        run.appendChild(ch.head);
        /* ⚠ v144 — THE ROW READS THE TREE, NOT THE LEVELS. Members
           arrived grouped by KIND (auto-countries, then containers,
           then hits), which printed "country zone zone subzone
           subzone" - every branch interleaved with every other. They
           are now walked depth-first from the head, so a chain reads
           the way it nests: Madagascar - Mahajanga - Melaky, then the
           next province and its own region. Siblings keep their
           arrival order, so nothing else about the row moves. */
        var byName = {};
        ch.members.forEach(function(m){ if (m.__place) byName[m.__place] = m; });
        /* the nearest ancestor PRESENT in this chain, else the head */
        function upTo(n){
          var cur = (HP[n] || {}).parent, guard = 0;
          while (cur && guard++ < 8){
            if (cur === c || byName[cur]) return cur;
            cur = (HP[cur] || {}).parent;
          }
          return c;
        }
        function depthOf(n){
          var d = 0, cur = n, guard = 0;
          while (cur && cur !== c && guard++ < 8){ cur = upTo(cur); d++; }
          return d;
        }
        var kids = {};
        ch.members.forEach(function(m){
          if (!m.__place) return;
          var p = upTo(m.__place);
          (kids[p] = kids[p] || []).push(m.__place);
        });
        var seq = [], guard = 0;
        (function walk(p){
          if (guard++ > 400) return;
          (kids[p] || []).forEach(function(n){ seq.push(n); walk(n); });
        })(c);
        /* anything the walk could not reach (a member whose container is
           missing from the feed) still ships, at the end */
        ch.members.forEach(function(m){
          if (m.__place && seq.indexOf(m.__place) < 0) seq.push(m.__place);
        });
        var deepest = 1;
        seq.forEach(function(n){ var d = depthOf(n); if (d > deepest) deepest = d; });
        var prev = c;
        seq.forEach(function(n){
          var m = byName[n];
          if (!m) return;
          var d = depthOf(n) || 1;
          m.className += " apsc-chip--linked";
          /* lines left to go: the first step of a two-deep chain wears
             two, its child wears one. Capped at three - past that the
             gauge stops being readable and the chain is a rarity. */
          m.setAttribute("data-lines", String(Math.max(1, Math.min(3, deepest - d + 1))));
          /* v147: A SIBLING IS NOT A RESTART (grower, 8.27.26: "why
             aren't all the new taiwanese geos from kiusianus linking to
             the country?"). The test was "is this pill a child of the
             pill immediately to its left" - which is true for the FIRST
             child of a country and false for every one after it. So a
             flat run of 14 Taiwanese counties drew ONE connector and 13
             blanks, and the row read as though only New Taipei belonged
             to Taiwan.
             The break worth marking is the walk popping back UP to a
             different branch (Madagascar > Mahajanga > Melaky, then
             Toliara), not the walk continuing across siblings. So a pill
             also keeps its tie when it shares a parent with the pill
             before it - the parent is on screen to their left either
             way, which is the whole claim the connector makes. */
          if (upTo(n) !== prev && upTo(n) !== upTo(prev))
            m.setAttribute("data-restart", "1");
          run.appendChild(m);
          prev = n;
        });
        distBox.appendChild(run);
      });
    ...
    (function(){
      ...
      function hidePop(){
        if (!pop) return;
        pop.classList.remove("apsc-chip-pop--on");
        setTimeout(function(){
          if (pop && !pop.classList.contains("apsc-chip-pop--on")) pop.style.display = "none";
        }, 220);
      }
      /* v136: the pop shows the TRUE CHAINS - heads, connectors,
         trimmed names, in chain order - not a flat pile of whatever
         got hidden. "As if the collapse never happened" (grower), and
         it fades in instead of jolting. Clones carry no listeners, so
         no map hover from inside the pop - the map is off-screen
         while pinned anyway. */
      function showPop(){
        if (!pop || !moreChip) return;
        var box = followBox.querySelector(".apsc-chips");
        pop.innerHTML = "";
        [].slice.call(box.children).forEach(function(node){
          if (node === moreChip) return;
          var cls = String(node.className || "");
          if (cls.indexOf("apsc-chip--continent") >= 0) return;
          var k = node.cloneNode(true);
          var inner = k.querySelectorAll ? [].slice.call(k.querySelectorAll(".apsc-chip")) : [];
          if (cls.indexOf("apsc-chip") >= 0) inner.push(k);
          inner.forEach(function(c){ c.style.display = ""; });
          pop.appendChild(k);
        });
        ...
        requestAnimationFrame(function(){ pop.classList.add("apsc-chip-pop--on"); });
      }
      function collapseChips(){
        ...
        /* v136: WHILE PINNED, ONLY THE CHAIN HEADS SURVIVE (grower:
           "the collapse should only show the highest level tag pills
           over the chains"). Subunits hide first - they are what the
           heads summarise - then heads tail-hide only if the row
           still wraps. */
        var hidden = 0;
        chips.forEach(function(c){
          if (String(c.className).indexOf("apsc-chip--linked") >= 0){
            c.style.display = "none"; hidden++;
          }
        });
        var heads = chips.filter(function(c){ return c.style.display !== "none"; });
        if (!moreChip){
          moreChip = el("span", "apsc-chip apsc-chip--more");
          box.appendChild(moreChip);
          pop = el("div", "apsc-chip-pop");
          pop.style.display = "none";   /* born hidden - see the CSS note */
        ...
        var rowTop = heads.length ? heads[0].offsetTop : 0;
        ...
        if (!hidden) moreChip.style.display = "none";
      }
  ...
  function boot(){
    ...
    mounts.forEach(function(mount){
      ...
      mount.setAttribute("data-apsc-version", "card-v150-file-v209");
      if (window.console && console.info){
...
(function(){
  ...
  var HUB_URL = "https://www.aroidpedia.com/aroid-reproduction";
  ...
  var LABELS = {
    ...
    "apai-src":    "Sources",
    /* ---- the Dieffenbachia reproduction page (new in v132) ----
       Five ticks. The opening block carries no h2 id, so the rail
       starts at p1, as on the Dracunculus and Homalomena pages.
       ⚠ apdi- is Dieffenbachia; apdr- is Dracunculus. */
    "apdi-p1":     "The three-day clock",
    "apdi-p2":     "Only scarabs",
    "apdi-p3":     "Abundance vs value",
    "apdi-p4":     "The tether",
    "apdi-src":    "Sources",

    /* ---- the Dracunculus reproduction page (new in v132) ----
       Seven ticks. The only one of the four new guides that is LIVE. */
    "apdr-p1":     "The plant",
    "apdr-p2":     "The heat",
    "apdr-p3":     "Who comes",
    "apdr-p4":     "The bait test",
    "apdr-p5":     "The amputations",
    "apdr-p6":     "The 1890 experiment",
    "apdr-src":    "Sources",

    /* ---- the Helicodiceros reproduction page (new in v132) ----
       Eleven ticks, the longest of the four.
       ⚠ aphe- is Helicodiceros; aphi- is Philodendron. */
    "aphe-p1":     "The plant",
    "aphe-p2":     "The two days",
    "aphe-p3":     "The smell",
    "aphe-p4":     "The heat",
    "aphe-p5":     "What the heat is for",
    "aphe-p6":     "Who comes",
    "aphe-p7":     "The trap",
    "aphe-p8":     "What the flies get",
    "aphe-p9":     "What it takes",
    "aphe-p10":    "The errors",
    "aphe-src":    "Sources",

    /* ---- the Homalomena reproduction page (new in v132) ----
       Nine ticks.
       ⚠ apho- is Homalomena; aphe- is Helicodiceros and aphi- is
       Philodendron. The genus code is the 2nd and 3rd letters. */
    "apho-p1":     "The chamber & clock",
    "apho-p2":     "Two systems",
    "apho-p3":     "The beetles",
    "apho-p4":     "Abundance vs value",
    "apho-p5":     "The flies inside",
    "apho-p6":     "The heat",
    "apho-p7":     "Resin & armour",
    "apho-p8":     "The scent",
    "apho-src":    "Sources",
