/* AROIDPEDIA footer bundle — built 2026-08-28 from FILE v181 by build-footer-bundle v1.
...
(function(){
  ...
  var VERSION = "apgm-v17-file-v181";
  ...
  function render(host, genus, shapes, geo, g, hier){
    ...
    Object.keys(HP).forEach(function(k){
      ...
      p.classList.add(depthOf(par) >= 1 ? "apgm-d2" : "apgm-d1");
    ...
    var hasDivisions = zones.concat(doubt).some(function(z){ return depthOf(z) >= 2; });
    ...
    function setHot(zone){
      ...
      if (currentView === "subzones" || currentView === "divisions"){
        var _ctr = !!PARENTS[zone] && (currentView === "divisions" || depthOf(zone) === 0);
        if (_ctr && residuals()[zone]){
          tail = " · " + residuals()[zone] + " not tagged to a " +
                 (currentView === "divisions" ? "division" : "subzone");
        }
      }
    ...
    function apply(view){
      ...
      var subView = view === "subzones" || divView;
      ...
      var countOf = function(z){
        var d = depthOf(z);
        if (subView && !divView && d === 1) return families()[z] || groupMap[z].count;
        if (subView && PARENTS[z]) return residuals()[z];
        return groupMap[z].count;
      };
      if (subView && !divView){
        order.forEach(function(z){
          if (depthOf(z) === 1 && groupMap[z]) groupMap[z] = { name: dispName(z), count: countOf(z) };
        });
      }
      /* ⚠⚠⚠ v16 — A CONTAINER NEVER SETS OR JOINS THE RAMP. v10 let a
         ...
         become a container. */
      var isCtr = function(z){ return !!PARENTS[z] && (divView || depthOf(z) === 0); };
      var max = 1;
      Object.keys(groupMap).forEach(function(z){
        if (subView && !divView && depthOf(z) >= 2) return;
      ...
      order.forEach(function(z){
        ...
        else if (subView && !divView && d >= 2) fo = 0;
        else if (subView && isCtr(z)) fo = d >= 1
                                       ? (residuals()[z] ? 0.28 : 0.18)
                                       : (residuals()[z] ? 0.16 : 0.07);
        else if (subView && !divView && d === 1) fo = ramp(countOf(z), max);
      ...
      if (view === "range"){
        ...
        legend.appendChild(el("span", "apgm__legend-t", "species per " +
          (view === "continents" ? "continent" :
           ...
           divView ? "division" : subView ? "state" : "zone")));
      }
    ...
    [["range", "Range"], ["continents", "Continents"], ["regions", "Regions"],
     ["countries", "Countries"],
     ["subzones", "States"], ["divisions", "Divisions"]].forEach(function(v){
    ...
    svg.addEventListener("mouseover", function(ev){
      ...
      if (paths[z] || (spaths[z] && groupMap[z])) setHot(z);
    ...
    svg.addEventListener("click", function(ev){
      ...
      openPanel(z, !paths[z] && !spaths[z]);   /* v17: a split is native ground */
    });
...
(function(){
  ...
  function boot(){
    ...
    mounts.forEach(function(mount){
      ...
      mount.setAttribute("data-apsc-version", "card-v147-file-v181");
      if (window.console && console.info){
