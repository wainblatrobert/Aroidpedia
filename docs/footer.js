/* AROIDPEDIA footer bundle — built 2026-08-21 from FILE v154 by build-footer-bundle v1.
...
(function(){
  ...
  window.__apFooterBundle = "v154";
  if (window.console && console.info) console.info("[footer bundle] FILE v154 (hosted footer.js)");
...
(function(){
  ...
  function buildMap(data, hits, doubtful, wash, subparent){
    ...
    var PATHBB = {}, BASEBB = [], SHAPEEL = {}, CONTBB = {};
    function bboxOfPath(d){
    ...
    order.forEach(function(name){
      ...
      if (_bb){
        ...
        /* v132: continent extents, for the WORLD button's new target */
        var _ct = (HIER3[name] || {}).continent;
        if (_ct){
          var _cb = CONTBB[_ct];
          if (!_cb) CONTBB[_ct] = _bb.slice();
          else { if(_bb[0]<_cb[0])_cb[0]=_bb[0]; if(_bb[1]<_cb[1])_cb[1]=_bb[1];
                 if(_bb[2]>_cb[2])_cb[2]=_bb[2]; if(_bb[3]>_cb[3])_cb[3]=_bb[3]; }
        }
      }
    ...
    /* v132: WORLD frames the CONTINENT, not the planet (grower: the
       whole earth says nothing about a West African species). Union of
       the continents the lit shapes belong to, plus a margin; a range
       spanning two continents gets both. Falls back to the old full
       earth only if no lit shape resolves to a continent. */
    var WORLDVB = null;
    (function(){
      var u = null;
      Object.keys(onSet).forEach(function(n){
        var c = (HIER3[n] || {}).continent;
        var b = c && CONTBB[c];
        if (!b) return;
        if (!u) u = b.slice();
        else { if(b[0]<u[0])u[0]=b[0]; if(b[1]<u[1])u[1]=b[1];
               if(b[2]>u[2])u[2]=b[2]; if(b[3]>u[3])u[3]=b[3]; }
      });
      if (!u) return;
      var w = u[2]-u[0], h = u[3]-u[1];
      WORLDVB = [u[0]-w*0.04, u[1]-h*0.04, w*1.08, h*1.08];
    })();
  ...
  function boot(){
    ...
    mounts.forEach(function(mount){
      ...
      mount.setAttribute("data-apsc-version", "card-v132-file-v154");
      if (window.console && console.info){
