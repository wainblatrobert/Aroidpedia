/* AROIDPEDIA footer bundle — built 2026-08-29 from FILE v188 by build-footer-bundle v1.
...
(function(){
  ...
  window.__apFooterBundle = "v188";
  if (window.console && console.info) console.info("[footer bundle] FILE v188 (hosted footer.js)");
...
(function(){
  ...
  var VERSION = "apgm-v24-file-v188";
  ...
  function render(host, genus, shapes, geo, g, hier){
    ...
    function apply(view){
      ...
      /* ⚠⚠⚠ v16 — A CONTAINER NEVER SETS OR JOINS THE RAMP. v10 let a
         ...
         become a container. */
      var isCtr = function(z){ return !!PARENTS[z] && (divView || depthOf(z) < rowD); };
      var max = 1;
      ...
      if (!subView && view !== "range" && hier && hier.places){
        ...
        Object.keys(byCG).forEach(function(c){
          ...
          if (!n) return;
          var fill;
          if (view === "countries") fill = ramp(n, max);
          else {
            var mem = byCG[c][0];
            fill = (groupMap[mem] && groupMap[mem].count)
                 ? ramp(groupMap[mem].count, max) : ramp(n, max);
          }
          ghostLit[c] = fill;
          groupMap[c] = { name: dispName(c), count: n, __ghost: 1 };
        });
      }
      order.forEach(function(z){
        var gr = groupMap[z];
        ...
        if (view === "range")   ;
                                       ...
                                       ? (residuals()[z] ? 0.28 : 0.18)
                                       : (residuals()[z] ? 0.16 : 0.07);
        ...
        if (gr && gr.merged && !cov && fo > 0){
          ...
          if (view === "range"){
            ...
            var rr = Math.round((19 + (175 - 19) * fo) * 0.84 + 239 * 0.16);
            var gg = Math.round((26 + (192 - 26) * fo) * 0.84 + 240 * 0.16);
            var bb2 = Math.round((21 + (144 - 21) * fo) * 0.84 + 232 * 0.16);
...
(function(){
  ...
  function boot(){
    ...
    mounts.forEach(function(mount){
      ...
      mount.setAttribute("data-apsc-version", "card-v148-file-v188");
      if (window.console && console.info){
