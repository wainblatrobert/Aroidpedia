/* AROIDPEDIA footer bundle — built 2026-08-30 from FILE v192 by build-footer-bundle v1.
...
(function(){
  ...
  var VERSION = "apgm-v28-file-v192";
  ...
  function render(host, genus, shapes, geo, g, hier){
    ...
    function apply(view){
      ...
      order.forEach(function(z){
        ...
        if (gr && gr.merged && !cov && fo > 0){
          /* v20 (#4): the seam colour leans 16% toward cream, so the
             boundary BETWEEN two groups reads as a faint two-tone line
             while seams inside a group stay near-invisible (grower:
             "light borders... they don't have to be very strong").
             Set as a custom property so the selection state can
             override it from CSS (#8). */
          if (view === "range"){
            /* v23: inside the one block the seam must vanish - the
               16% cream lean is for boundaries BETWEEN groups. */
            paths[z].style.setProperty("--mst", "rgb(144,159,119)");
          } else {
            /* v25: the cream lean draws borders - wanted between
               countries, unwanted inside a continent or region block
               (grower: "only continents should be visible"). */
            var LEAN = view === "countries" ? 0.16 : 0;
            var rr = Math.round((19 + (175 - 19) * fo) * (1 - LEAN) + 239 * LEAN);
            var gg = Math.round((26 + (192 - 26) * fo) * (1 - LEAN) + 240 * LEAN);
            var bb2 = Math.round((21 + (144 - 21) * fo) * (1 - LEAN) + 232 * LEAN);
            paths[z].style.setProperty("--mst", "rgb(" + rr + "," + gg + "," + bb2 + ")");
          }
        } else {
...
(function(){
  ...
  function boot(){
    ...
    mounts.forEach(function(mount){
      ...
      mount.setAttribute("data-apsc-version", "card-v148-file-v192");
      if (window.console && console.info){
