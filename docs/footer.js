/* AROIDPEDIA footer bundle — built 2026-08-16 from FILE v66 by build-footer-bundle v1.
...
(function(){
  ...
  function render(mount, parsed, facts, data){
    ...
    if (!followBox.querySelector(".apsc-map") && !followBox.querySelector(".apsc-fact")){
      ...
      followBox = null;
      /* (the v50 stuck-strip toggle below no-ops when there is no
         follow panel) */
    }
    /* v50: the seam strips render only while the panel is actually
       pinned — see the CSS. Stuck = the sticky panel's top displaced
       from its cell's top (it stays put while the cell scrolls up). */
    if (followBox){
      (function(fol, cell){
        var raf = false;
        function upd(){
          raf = false;
          if (!fol.isConnected) return;
          var d = fol.getBoundingClientRect().top - cell.getBoundingClientRect().top;
          fol.classList.toggle("apsc-follow--stuck", d > 6);
        }
        window.addEventListener("scroll", function(){
          if (!fol.isConnected || raf) return;
          raf = true;
  ...
  function boot(){
    ...
    mounts.forEach(function(mount){
      ...
      mount.setAttribute("data-apsc-version", "card-v50-file-v66");
      if (window.console && console.info){
