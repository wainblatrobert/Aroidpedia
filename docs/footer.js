...
(function(){
  ...
  var VERSION = "apgm-v29-file-v193";
      ...
      Object.keys(ghostPaths).forEach(function(k){
        ...
        if (!subView){
          if (ghostLit[k]){
            ...
            el.style.setProperty("--mst", compFill(ghostLit[k]));
          }
          return;
        }
...
(function(){
  ...
  function parse(layout, selfBlock){
    ...
    blocks.forEach(function(b, i){
      ...
      if (html){
        blockLines(html).forEach(function(node, li){
          ...
          var at = i + (li + 1) / 1000;
          /* v149: STORY terminator - "/STORY", "END STORY:" or
             "STORY END:" closes the story span on manual posts, so
             what follows is the post again, not more story. Checked
             before headingOf and without the bold requirement: an
             end marker is punctuation, not a section. Only honoured
             while a story is actually open - anywhere else the line
             stays visible prose, which is the honest failure. */
          if (cur && cur.key === "story" &&
              /^(\/\s*STORY|END\s+STORY|STORY\s+END)\s*:?\s*$/i.test(squash(txt(node)))){
            cur = open({key:"notes", label:null});
            return;
          }
    ...
    if (lastContentBlock >= 0){
      order.forEach(function(k){
        /* v149: a photo inside the STORY span is a story plate, never
           the post's own grid - exempt from the Lutea rule. */
        if (k === "story") return;
        var sec = sections[k];
  ...
  function render(mount, parsed, facts, data){
    ...
    /* v149: manual posts - image blocks pasted inside the STORY span
       are the plates when no manifest supplies a story role. Same
       record shape (src/alt/cap/capHtml/dim), so the plate picker and
       the lightbox need no changes. Manifest plates still win. */
    if (!storyPlates.length && S.story && S.story.images && S.story.images.length){
      storyPlates = S.story.images;
    }
    ...
    var allImgs  = [];
    /* v149: story-span images are plates, not archive - keep them out
       of the hero pick and the More photos pool, exactly as manifest
       story plates always were. */
    parsed.order.forEach(function(k){ if (k === "story") return; allImgs = allImgs.concat(S[k].images); });
  ...
  function boot(){
    ...
    mounts.forEach(function(mount){
      ...
      mount.setAttribute("data-apsc-version", "card-v149-file-v194");
      if (window.console && console.info){
