...
(function(){
  ...
  ex.src = "https://assets.squarewebsites.org/lazy-summaries/lazy-summaries.min.js";
  ex.defer = true;
  (document.head || document.documentElement).appendChild(ex);
})();
;
...
(function(){
  ...
  function render(host, genus, shapes, geo, g){
    ...
    function selReadout(){
      if (!selEp) return;
      var zs = (g.speciesPlaces[selEp] || []);
      var drawn = zs.filter(function(z){ return paths[z]; });
      hover.setAttribute("data-on", "1");
      hover.innerHTML = "";
      var em = document.createElement("em");
      em.textContent = genus + " " + selEp;   /* never contract a genus */
      hover.appendChild(em);
      hover.appendChild(document.createTextNode(" · present in " + drawn.length +
        (drawn.length === 1 ? " region" : " regions") +
        (zs.length > drawn.length ? " (+" + (zs.length - drawn.length) + " unmapped)" : "")));
    }
    function clearSel(){
      selEp = null;
      svg.classList.remove("apgm--sel");
      order.forEach(function(z){ paths[z].classList.remove("is-sel"); });
      if (searchUI){
    ...
    function selectEp(ep){
      ...
      var zs = {};
      (g.speciesPlaces[ep] || []).forEach(function(z){ zs[z] = 1; });
      svg.classList.add("apgm--sel");
      order.forEach(function(z){ paths[z].classList.toggle("is-sel", !!zs[z]); });
      if (searchUI){
    ...
    function apply(view){
      ...
      groupMap = groupsFor(view === "countries" ? "zones" : view);
      var max = 1;
      ...
      order.forEach(function(z){
        var gr = groupMap[z];
        /* v6: subunits are outline-only detail — the L3 parent under
           them carries the choropleth, and stacking a second fill
           would paint Sarawak brighter than its own count */
        paths[z].style.fillOpacity = SUB[z] ? 0
          : (view === "range" ? 0.8 : ramp(gr.count, max));
    ...
    [["range", "Range"], ["continents", "Continents"], ["regions", "Regions"],
     ["countries", "Countries"], ["zones", "Zones"]].forEach(function(v){
...
(function(){
  ...
  function parse(layout, selfBlock){
    ...
    var pre      = { nodes:[], images:[], videos:[] };  /* anything before the first heading */
    var hidden   = [];

    function open(h){
      if (!sections[h.key]){
        sections[h.key] = { key:h.key, label:h.label, nodes:[], images:[], videos:[], subs:[] };
        order.push(h.key);
      }
      cur = sections[h.key];
      return cur;
    ...
    blocks.forEach(function(b, i){
      ...
      if (isCode && !CFG.HIDE_CODE_BLOCKS) return;  /* leave injections alone, do not parse them */
  ...
  function resolveRegions(tags, distText, data){
    ...
    function tryName(name){
      ...
      /* case-insensitive lookup against the shape keys */
      var match = null;
      for (var s in known){ if (s.toUpperCase() === canon.toUpperCase()){ match = s; break; } }
      if (!match) for (var dn in dots){ if (dn.toUpperCase() === canon.toUpperCase()){ match = dn; break; } }
      seen[up] = 1;
      if (match){
        hits.push(match);
  ...
  var apscHatchSeq = 0;
  function buildMap(data, hits, doubtful){
    ...
    pat.appendChild(pr); pat.appendChild(pl);
    ...
    /* frame on the highlighted regions */
    var bb = null;
    hits.forEach(function(name){
    ...
    if (bb){
      ...
      worldBtn.addEventListener("click", function(){
        wide = !wide;
        svg.setAttribute("viewBox", (wide ? full : zoom).join(" "));
        worldBtn.textContent = wide ? "Close in" : "World";
      });
      wrap.appendChild(worldBtn);
    }
  ...
  function notesList(nodes){
    ...
    if (!kids.length || !/^1[.)]\s/.test(squash(txt(kids[0])))) return null;
    var ol = el("ol","apsc-notes");
    var li = null;
    kids.forEach(function(k){
      var m = /^(\d+)[.)]\s+/.exec(squash(txt(k)));
      if (m){
        li = el("li");
        li.setAttribute("value", m[1]);
  ...
  function render(mount, parsed, facts, data){
    ...
    var mapNode = buildMap(data, geo.hits, geo.doubtful);
    ...
    if (!distNote && S.distribution && distText && !isNA(distText)){
      ...
      if (resid.replace(/[^A-Za-z]/g, "").length >= 24){
        var lgc = el("div");
    ...
    if (distBox){
      ...
      fact("Distribution", distVal, followBox);
    }
    ...
    SCHEMA.forEach(function(def){
      ...
      if (!hasText && !hasSubs){ na.push(def.label); return; }
  ...
  function boot(){
    ...
    mounts.forEach(function(mount){
      ...
      mount.setAttribute("data-apsc-version", "card-v49-file-v64");
      if (window.console && console.info){
      ...
      Promise.all([shapes(), photoManifest()]).then(function(res){
        try {
          mergeManifest(parsed, res[1], facts.title);
          fillAlt(parsed, facts.title);
          render(mount, parsed, facts, res[0]);
        }
        catch (err){
...
(function(){
  ...
  function buildRow(res, version, placeCount){
    var row = el("div", "apsc-fact apsc-fact--clim");
    var climLabel = el("div", "apsc-fact__label", "Climate range");
    ...
    var rowT = el("div", "apsc-clim__row");
    ...
    var readT = el("div", "apsc-clim__read");
  ...
  function augment(){
    ...
    [].forEach.call(mounts, function(mount){
      ...
      /* solid chips only: --off has no shape (and so no climate entry),
         ...
         record and must not shape the climate envelope (v50) */
      var chips = distRow.querySelectorAll("a.apsc-chip:not(.apsc-chip--off):not(.apsc-chip--continent):not(.apsc-chip--doubtful)");
