/* AROIDPEDIA footer bundle — built 2026-08-19 from FILE v108 by build-footer-bundle v1.
...
(function(){
  ...
  window.__apFooterBundle = "v108";
  if (window.console && console.info) console.info("[footer bundle] FILE v108 (hosted footer.js)");
...
(function(){
  ...
  function buildMap(data, hits, doubtful, wash, subparent){
    ...
    order.forEach(function(name){
      ...
      if (subparent[name] && !onSet[name] && !ctxSet[name]){
        var ps = subparent[name] || [], live = false;
        for (var pi = 0; pi < ps.length; pi++){
          if (onSet[ps[pi]] || ctxSet[ps[pi]]){ live = true; break; }
        }
        if (!live) return;
      }
    ...
    if (bb){
      ...
      function zoomBy(f){
        ...
        /* never wider than the world, never past MINW */
  ...
  function boot(){
    ...
    mounts.forEach(function(mount){
      ...
      mount.setAttribute("data-apsc-version", "card-v90-file-v108");
      if (window.console && console.info){
