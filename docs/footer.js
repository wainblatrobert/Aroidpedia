/* AROIDPEDIA footer bundle — built 2026-08-21 from FILE v151 by build-footer-bundle v1.
...
(function(){
  ...
  window.__apFooterBundle = "v151";
  if (window.console && console.info) console.info("[footer bundle] FILE v151 (hosted footer.js)");
...
(function(){
  ...
  function buildMap(data, hits, doubtful, wash, subparent){
    ...
    (function(){
      ...
      var entries = [];
      /* v129: the island counts however it arrived. The trigger was
         islandSet alone, which only fills when the island is a
         GRANDPARENT of a lit unit - so baginda (East Kalimantan, two
         hops) got labels while brachyphyllus (Sarawak, one hop, island
         in the ORDINARY WASH) got none. Same island, same reader, same
         need. Any ISLANDS member present as island depth, wash or lit
         now switches the map to shape-mode labels. */
      var islQ = {};
      Object.keys(ISLANDS).forEach(function(k){
        if (islandSet[k] || ctxSet[k] || onSet[k]) islQ[k] = 1;
      });
      if (Object.keys(islQ).length){
        Object.keys(onSet).forEach(function(n){
          if (PATHBB[n] && !islQ[n]) entries.push({ text:n, bb:PATHBB[n], shapes:[n] });
        });
      ...
      if (entries.length < 2 || entries.length > CFG.CLAB_MAX) return;
  ...
  function boot(){
    ...
    mounts.forEach(function(mount){
      ...
      mount.setAttribute("data-apsc-version", "card-v129-file-v151");
      if (window.console && console.info){
