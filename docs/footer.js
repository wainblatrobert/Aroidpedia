/* AROIDPEDIA footer bundle — built 2026-08-21 from FILE v142 by build-footer-bundle v1.
...
(function(){
  ...
  window.__apFooterBundle = "v142";
  if (window.console && console.info) console.info("[footer bundle] FILE v142 (hosted footer.js)");
...
(function(){
  ...
  function buildMap(data, hits, doubtful, wash, subparent){
    ...
    function sizeDots(){
      ...
      /* Everything here is a FRACTION OF THE VIEWBOX, which is the same
         thing as a fraction of the rendered map: r_user = k * vbWidth
         renders at k * svgWidthPx, whatever the zoom or the layout
         width. So the marker is proportional to the map on a phone and
         on a 1728px desktop alike, and does not grow when you zoom. */
      var W = vb[2];
      var rr   = W * 0.0072;   /* ~3px radius on a 420px map */
      var lead = W * 0.075;    /* leader length - long enough to READ as a
                                  line pointing at the place, which is the
                                  whole reason the label is set off */
      var fs   = W * 0.024;    /* label type size */
  ...
  function boot(){
    ...
    mounts.forEach(function(mount){
      ...
      mount.setAttribute("data-apsc-version", "card-v121-file-v142");
      if (window.console && console.info){
