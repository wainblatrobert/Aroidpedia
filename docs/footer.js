/* AROIDPEDIA footer bundle — built 2026-08-27 from FILE v180 by build-footer-bundle v1.
...
(function(){
  ...
  window.__apFooterBundle = "v180";
  if (window.console && console.info) console.info("[footer bundle] FILE v180 (hosted footer.js)");
...
(function(){
  ...
  var VERSION = "apgm-v16-file-v173";
  ...
  function render(host, genus, shapes, geo, g, hier){
    ...
    function depthOf(z){
      var d = 0, cur = z;
      while (SUB[cur] && d < 6){ cur = SUB[cur]; d++; }
      return d;
    }
    ...
    Object.keys(HP).forEach(function(k){
      var par = HP[k].parent;
      if (!par || !PARENTS[par] || paths[k] || dpaths[k] || !shapes.shapes[k]) return;
      var p = svgEl("path");
    ...
    function chainOf(z){
      var out = [], cur = z;
      for (var i = 0; i < 5 && SUB[cur]; i++){ cur = SUB[cur]; out.push(cur); }
      return out;
    }
    var RESID = null;
    function residuals(){
    ...
    function groupsFor(view){
      ...
      if (view === "countries"){
        ...
        if (HC){
          var byC = {}, home = {};
          zones.forEach(function(z){
            if (SUB[z]) return;             /* subunits ride their zone */
    ...
    function apply(view){
      ...
      if (subView && !divView){
        order.forEach(function(z){
          if (depthOf(z) === 1 && groupMap[z]) groupMap[z] = { name: z, count: countOf(z) };
        });
      }
      ...
      if (view === "range"){
        ...
        legend.appendChild(el("span", "apgm__legend-t", "species per " +
          (view === "continents" ? "continent" :
           view === "regions" ? "region" :
           divView ? "division" : subView ? "subzone" : "zone")));
      }
    ...
    [["range", "Range"], ["continents", "Continents"], ["regions", "Regions"],
     ...
     ["subzones", "Subzones"], ["divisions", "Divisions"]].forEach(function(v){
    ...
    svg.addEventListener("mouseover", function(ev){
      ...
      if (paths[z]) setHot(z); else if (ghosts[z]) setGhost(z); else setDoubt(z);
    });
...
(function(){
  ...
  function boot(){
    ...
    mounts.forEach(function(mount){
      ...
      mount.setAttribute("data-apsc-version", "card-v147-file-v179");
      if (window.console && console.info){
