/* AROIDPEDIA footer bundle — built 2026-09-01 from FILE v204 by build-footer-bundle v1.
...
(function(){
  ...
  window.__apFooterBundle = "v204";
  if (window.console && console.info) console.info("[footer bundle] FILE v204 (hosted footer.js)");
...
(function(){
  ...
  var VERSION = "apgm-v40-file-v204";
  ...
  function render(host, genus, shapes, geo, g, hier){
    ...
    var climP = null, climBuilt = 0, climPresent = null, climFailed = 0;
    ...
    function climBuild(im){
      if (climBuilt || !im) return;
      climBuilt = 1;
      var ds = order.filter(function(z){ return paths[z] && paths[z].getAttribute("d"); })
                    .map(function(z){ return paths[z].getAttribute("d"); });
      var cid = "apgm-climclip-" + Math.round(HOME[2] * 100) + "-" + order.length;
      var defs = svgEl("defs"), cp = svgEl("clipPath");
      cp.setAttribute("id", cid);
      ds.forEach(function(d){ var q = svgEl("path"); q.setAttribute("d", d); cp.appendChild(q); });
      defs.appendChild(cp); climHost.appendChild(defs);
      var img = svgEl("image");
      ...
      /* country borders ON TOP, clipped to the same range: inside a
         band a reader has nothing to place themselves by (grower). */
    ...
    function apply(view){
      ...
      if (climOn){
        climLoad().then(function(im){ climBuild(im); climLegend(); });
      }
      ...
      if (!subView && view !== "range" && !climOn && hier && hier.places){
      ...
      order.forEach(function(z){
        ...
        var cov = !subView && view !== "range" && !climOn && coveredBy(z);
      ...
      Object.keys(ghostPaths).forEach(function(k){
        ...
        if (!subView){
          if (ghostLit[k]){
            ...
            el.style.setProperty("--zf", compFill(ghostLit[k]));
...
(function(){
  ...
  function boot(){
    ...
    mounts.forEach(function(mount){
      ...
      mount.setAttribute("data-apsc-version", "card-v148-file-v204");
      if (window.console && console.info){
