/* AROIDPEDIA footer bundle — built 2026-08-29 from FILE v186 by build-footer-bundle v1.
...
(function(){
  ...
  window.__apFooterBundle = "v186";
  if (window.console && console.info) console.info("[footer bundle] FILE v186 (hosted footer.js)");
...
(function(){
  ...
  var VERSION = "apgm-v22-file-v186";
  ...
  function render(host, genus, shapes, geo, g, hier){
    ...
    function covPaints(c, z){
      if (!c || c === z) return false;
      var el = paths[c] || dpaths[c];
      var viaGhost = false;
      if (!el && covGhostLit[c] && ghostPaths[c]){
        el = ghostPaths[c];                    /* v22: a lit ghost paints */
        viaGhost = true;
      }
      if (!el) return false;
      if (!viaGhost && paths[c] && SUB[c]) return false;  /* fill-0 in the flat views */
      var pts = covProbe(z);
      if (!pts) return !paths[z];              /* no geometry: paint if we
                                                  have a path, defer if not */
      var inC = 0;
      try {
        for (var i = 0; i < pts.length; i++){
          if (el.isPointInFill(pts[i])) inC++;
        }
      } catch (e) { return true; }
      return inC * 2 > pts.length;             /* the country holds most of
                                                  the unit's ground */
    }
    function coveredBy(z){
      if (covCache[z] !== undefined) return covCache[z];
      var hit = covPaints((HP[z] || {}).country, z);
      var m = MULTI[z];
      if (!hit && m && m.countries){
        hit = m.countries.some(function(c){ return covPaints(c, z); });
      }
      covCache[z] = hit;
      return hit;
    }
    ...
    function apply(view){
      ...
      order.forEach(function(z){
        var gr = groupMap[z];
        var fo, d = depthOf(z);
        var cov = !subView && coveredBy(z);          /* v20 (#1) */
        paths[z].classList.toggle("apgm-cov", cov);
        if (view === "range")   fo = (SUB[z] || cov) ? 0 : 0.8;
...
(function(){
  ...
  function boot(){
    ...
    mounts.forEach(function(mount){
      ...
      mount.setAttribute("data-apsc-version", "card-v148-file-v186");
      if (window.console && console.info){
